...
| 10.6  | -           | **이카운트 → Samhan Public 마이그레이션** (MIG-1~15 완료) + **MIG-16 BE Minor 청소** (partnerNames batch / AGING pagination / 권한 flash / refresh toast) | **MIG-16 진행 중 — 옵션 C 21단계** |
...
- MIG-16 진행 중: partner-service batch lookup, accounting admin partnerName batch, aging snapshot pagination, refresh toast, 권한 로딩 flash 방지를 정리한다.
- 다음 후보: SP-08-5-3 매입 soft delete + InboundInspection 정합, SP-08 회계/vendor OCR/Aligo 후속 parity, MIG-16 검증 완료 후 운영 데이터 실 import 검증.
...
- **MIG-15** (진행 중, 본 PR) — **POI shared/common → shared/ecount-io module 분리** — `shared:ecount-io` 신규 module을 추가하고 `EcountXlsxSupport` + `ExcelExporter` POI 구현을 이동한다. `shared/common`은 POI 비의존 DTO/exception 중심으로 되돌리고, accounting/partner는 direct POI 선언을 제거한다. arologis/slip/inventory는 자체 POI 사용(`VendorExcelParser` / `SlipExcelExportIT` / `DpsExcelParser`) 때문에 direct dependency를 유지한다.
