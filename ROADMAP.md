...
- SP-08-3-3: slip 전표정리 저장내역 구현 진행. `slip_cleanup_save_history` JSONB 저장내역과 `/slips/cleanup/history` 4 endpoint를 추가하고, `/sales/slip-cleanup` 화면에 실행/저장내역 2탭과 latest 자동 복원/명시 저장을 연결한다.
- 다음 후보: SP-08-3-4 배차문자 preview/send audit history.
