...
- **MIG-7** (본 PR) — **Cash 도메인 신규 + MIG-5 staging 변환** — accounting-service V27 `cash_disbursements` / `cash_receipts` 도메인, 지출결의서/입금보고서 staging transform endpoint 2종, auth-service V20 PageCode 2종, shared/common MIG7 ErrorCode 6종을 추가한다. Journal 자동 생성은 MIG-8+로 이연한다.
