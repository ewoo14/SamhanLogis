...
- **MIG-3** (진행 중) — **이카운트 회계 전표 4종 마이그레이션** — MIG-1 PoC + MIG-2 5 importer + SAS 시리즈 5 패턴을 미러해 accounting-service에 V23 staging 4종, 4 importer/controller, partner-service name lookup, `staging.ecount_account_map` 역방향 계정 lookup, 회계전표분개 차/대 균형 POSTED/DRAFT 분기를 추가한다. auth-service V16은 MIG3 PageCode 4종과 MASTER/MANAGER edit seed를 포함한다.
