...
- **MIG-5** (진행 중) — **이카운트 창고이동·지출결의서·입금보고서 raw 3종 마이그레이션** — inventory-service V13 창고이동 staging + `StockTransfer` 도메인 변환, accounting-service V25 지출결의서/입금보고서 staging only + Partner aging cross-check, auth-service V18 PageCode 3종, shared/common MIG5 ErrorCode 8종을 추가한다.
