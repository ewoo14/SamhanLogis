...
- 🔄 **2026-07-18 supersede(#825 슬4 D-S4-02·PR #844)**: "각자 hand-roll·신규 컴포넌트 금지" → **두 primitive 조합·패키지한 표준 컴포넌트 `MultiSelectAutocomplete`(엔티티) / `FreeTextChipInput`(임의 문자열)로 통일**(재발명 아님=TagChip+AsyncAutocomplete 조합 유지). 결재작성·결재선설정(각자 hand-roll)→MultiSelectAutocomplete 표준화, 결재양식 SELECT 옵션(CSV)→FreeTextChipInput. delta API(onAdd/onRemove)·filter-selected(멀티 ARIA)·prefill 경합/순서 보존·opaque DOM id(슬3) 상속. CODEF=슬5 이관(D-S4-05). → [[feedback_reconvergence_before_merge]]·[[feedback_design_system_playwright_mock_suite]]
