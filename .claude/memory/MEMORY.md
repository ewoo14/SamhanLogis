...
- [🚨 표준 워크플로우 유일 진실원 (07-15 개편 · 07-20 기획검수 폐지 · 🚨07-20 2차 리뷰=FABLE5·fix=SONNET5)](feedback_canonical_workflow.md) — **OPUS 4.8 기획**(조기 PR 개설+기획 리뷰 게시) → **CODEX LUNA 5.6 구현**(게시) → (**FABLE5 5+agent 적대리뷰·라이브QA** + **SONNET5 fix**+검증+게시 → **CODEX SOL 5.6 5+agent 리뷰**+**LUNA fix**+게시) **0수렴까지 반복** → PM 종합(게시) → CI green → PM 머지. **OPUS 4.8=기획+PM 오케스트레이션 전담(리뷰·라운드 fix 금지)**. 🚫엄수·단축금지·순차(병렬금지)·**모든 단계 리뷰 게시(실행=게시 1:1)**·**라이브QA 스샷 다수 필수**·fix=라운드모델·false-positive 의심도 재수렴+PM종합 후·모델 대체 금지(부재 시 개발책임자 선확인)·9-게이트·근본원인 자각(속도<충실도)·매단계 ScheduleWakeup. 모델 ID 실측: `gpt-5.6-sol`/`gpt-5.6-luna`. **🚨2026-07-20 기획검수(구 CODEX SOL spec검수 2단계) 폐지=적대리뷰 중복·spec점검은 OPUS 기획 흡수**. **🚨2026-07-20 2차(현행 정본): 1차 적대검증 리뷰=FABLE5(`model:"fable"`)·그 라운드 fix=SONNET5(`model:"sonnet"`)·OPUS 4.8=기획+PM 오케스트레이션 전담(리뷰·라운드 fix 직접 수행 금지)**. ※과거 이력: 07-16 에 FABLE5 를 폐지하고 OPUS 겸임으로 갔던 결정은 **07-20 2차 지시로 번복·복원**됨. **구 워크플로우 전부 폐기**
