...
- [Claude 기획 → Codex 개발 → 양쪽 review/fix 사이클 → PM 자동 머지 → 다음 PR 자동](feedback_dual_5agent_review.md) — 2026-05-19 사용자 정정 (9회차 최종). 5-단계: Claude 기획 → **Codex 개발** → (Claude 5-agent review 게시+fix → Codex 5-agent review 게시+fix) × **최대 N=3 사이클 모든 오류 fix 의무** → **이상 없을 경우 PM 자동 머지 + 다음 PR 자동 진행**. Claude subagent 단독 implementation 금지. **QA agent 는 Docker 실서버 직접 테스트 의무**. 후속 PR 백로그 금지.
