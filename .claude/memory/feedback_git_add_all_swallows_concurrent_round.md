...
## 🚨 2026-08-18 — **경로를 좁히면 이번엔 빠뜨린다.** 커밋 후 `git status` 로 잔여 0 을 확인하라

위 2번(경로 명시)을 지켰더니 반대쪽으로 틀렸다.

```text
PM 커밋   git add -A -- clients services docs
그 라운드가 건드린 곳   clients · services · docs · 🚨scripts
⟹ scripts/ 2파일·3줄이 워크트리에만 남고 PR SHA 에는 안 올라갔다

적대검증 4차가 잡았다 — "fix3 반영 누락: 2파일·3줄이 PR SHA 에 없음"
🚩 그 라운드의 결함 4건 중 3건이 이 커밋 실수였다
   검증자는 브랜치를 보지 워크트리를 보지 않는다
```
...
```bash
git -C <wt> add -A -- <경로들>
git -C <wt> commit -F <메시지파일>
git -C <wt> push -q origin <브랜치명>
git -C <wt> status --porcelain     # 🚨 여기가 비어야 끝난다
```

```text
🚨 비어 있지 않으면 그 파일이 무엇인지 보고 판단하라
     이 라운드가 만든 것       → 빠뜨렸다. 추가 커밋하라
     다른 라운드가 만든 것     → 정상. 그대로 두라
🚨 발주 브리핑에 "변경 파일 목록" 을 요구해 두면 대조할 기준이 생긴다
🚨 push 뒤에는 브랜치 이름을 명시하라 — 워크트리가 detached 면 `HEAD` 로 밀다 실패한다
   (2026-08-18 실측: wdcp 가 detached 여서 push 가 거부됐다.
    같은 브랜치를 wdc02 가 잡고 있던 것이 원인 — 중복 워크트리를 만들지 마라)
```

🔑 **`-A` 는 남의 것을 삼키고, 경로 좁히기는 내 것을 빠뜨린다.** 어느 쪽을 택하든 **커밋 뒤 `git status` 로 확인**하는 것이 유일한 안전장치다.
