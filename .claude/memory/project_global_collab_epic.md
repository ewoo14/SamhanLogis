...
**다음 슬라이스 = 그룹웨어 결재 collab** (개발책임자 2026-06-14 확정). **🔑 도메인 규칙: 최종 결재가 완료된 문서(전표)는 수정 불가** → collab `guardCollabModifiable` COLLAB_LOCKED = 최종결재완료 상태(409). 결재 진행 중/반려 상태의 수정 허용 여부는 슬라이스 진입 시 정찰·확정. **⚠️ 단 그 전에 KST(Asia/Seoul) 전역화 전담 PR 우선**([[project_kst_timezone_standard]] — 개발책임자 2026-06-14 지시, 현재 진행 중 feat/kst-timezone-standard).
