...
**🚨 여섯 번째 변종 — 병렬 슬라이스의 Flyway 마이그레이션 번호 경합 (2026-07-23 #907 실증):** 두 트랙이 **같은 서비스에 마이그레이션을 추가**하면 번호가 충돌한다. #824 가 먼저 머지되며 `V12__add_partner_order_line_supply_vat.sql` 를 선점했는데, 5커밋 뒤처진 #907 브랜치도 `V12__add_partner_identity_to_orders.sql` 를 썼다. 증상은 **SQL 이 아니라 스프링 컨텍스트 로드 전멸**로 나타난다 — `org.flywaydb.core.api.FlywayException at CompositeMigrationResolver.java:92` 가 `ApplicationContextLoadIT`·`TaskSchedulerPoolSizeIT`·`CloudWatchMetricsConfig*IT` 를 한꺼번에 FAILED 로 만들어 원인이 마이그레이션임이 로그 첫 줄에 안 보인다.

🔑 **로컬 fresh Postgres probe 는 이 충돌을 구조적으로 못 잡는다** — 자기 브랜치의 migration 디렉터리만 보기 때문이다. #907 은 probe `FRESH_PROBE_PASS` 를 받고도 CI 에서 죽었다. **권위는 `origin/main` 을 병합한 뒤의 CI 다.**

**How to apply:** ① 병렬 트랙이 같은 서비스를 건드리면 **착수 시점에 `git ls-tree origin/main -- <svc>/db/migration` 로 다음 번호를 확인**하고, 머지 직전 한 번 더 확인한다. ② 충돌 시 **미머지 마이그레이션을 개명**한다(아직 어디에도 적용된 적 없으므로 [[feedback_applied_migration_immutable]] 에 저촉되지 않음 — 이미 머지·적용된 것은 여전히 불변). ③ 개명 시 **파일 헤더 주석과 dev-report 의 버전 참조까지** 함께 고친다(둘 다 실제로 V12 를 박아 두고 있었다). ④ 개명 후 **main 병합 상태에서 fresh probe 재실행** — 두 마이그레이션이 같은 테이블을 건드리면 순서 의존이 생길 수 있다(#907·#824 는 `partner_orders` vs `partner_order_lines` 로 무관했으나 확인은 해야 한다). ⑤ CI 실패를 볼 때 `gh pr checks` 의 이름만 보지 말고 **실패 job 의 로그 본문**을 읽을 것 — "빌드 + 테스트 (accounting+partner)" 라는 이름만으로는 Flyway 충돌임을 알 수 없다.
