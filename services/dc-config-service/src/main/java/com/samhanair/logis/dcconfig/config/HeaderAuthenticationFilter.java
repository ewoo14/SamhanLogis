...
public class HeaderAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        if (isPublic(request) || request.getRequestURI().startsWith("/internal/") || isInternalPrincipal()) { chain.doFilter(request,response); return; }
        ...
        if ((userId == null || userId.isBlank()) && groups != null && !groups.isBlank()) { response.setStatus(HttpServletResponse.SC_UNAUTHORIZED); return; }
        if (userId != null && !userId.isBlank() && SecurityContextHolder.getContext().getAuthentication() == null) {
            List<SimpleGrantedAuthority> authorities=new ArrayList<>();
            if (groups != null && !groups.isBlank()) for (String group: groups.split(",")) if (!group.isBlank()) authorities.add(new SimpleGrantedAuthority("GROUP_"+group.trim()));
