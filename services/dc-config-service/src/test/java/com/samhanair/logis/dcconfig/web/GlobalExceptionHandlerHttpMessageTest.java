...
/** dc-config-service JSON body 파싱 실패 응답 계약 테스트. */
class GlobalExceptionHandlerHttpMessageTest {

    private final MockMvc mockMvc = MockMvcBuilders
            .standaloneSetup(new InternalDcConfigController(
                    mock(DcConfigService.class),
                    mock(PriceCalculationService.class),
                    mock(EstimateConfigService.class)))
            .setControllerAdvice(new GlobalExceptionHandler())
            .build();

    /** 실제 가격 계산 POST 의 깨진 JSON body 는 400 INVALID_INPUT 으로 고정한다. */
    @Test
