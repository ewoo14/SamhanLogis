...
`EXPO_PUBLIC_APP_VERSION`을 명시적으로 주입할 수 있으며, 누락 시 저장소의 공통 빌드 버전
해석기가 KST 날짜·빌드번호로 `YYYY/MM/DD-{번호}`를 생성하고 경고한다. 따라서 `0.0.0`은
정책 버전으로 사용하지 않는다. 패키지 semver는 마켓·번들 도구 식별자로만 남긴다.
