...
@Entity
@Getter
@Table(name = "purchase_accounting_slip_lines",
       uniqueConstraints = @UniqueConstraint(columnNames = {"slip_id", "line_no"}))
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@SQLRestriction("is_deleted = false")
public class PurchaseAccountingSlipLine extends BaseEntity {
    ...
    private PurchaseAccountingSlip slip;
    ...
    // IN(...) 배치 조회한다.
    @BatchSize(size = 100)
    @OneToMany(mappedBy = "purchaseSlipLine", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<PurchaseAccountingSlipAllocation> allocations = new ArrayList<>();

    public static PurchaseAccountingSlipLine create(PurchaseAccountingSlip slip, int lineNo,
            String productCode, String productName,
            BigDecimal qty, BigDecimal unitPrice,
            BigDecimal supplyAmount, BigDecimal vatAmount, BigDecimal lineTotal) {
        PurchaseAccountingSlipLine l = new PurchaseAccountingSlipLine();
        l.slip = slip;
        l.lineNo = lineNo;
