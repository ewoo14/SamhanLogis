...
/** MIG-4 — Admin 이카운트 주문서 CSV import. */
@Slf4j
@RestController
@RequestMapping("/admin/accounting/orders/imports")
@RequiredArgsConstructor
@Tag(name = "MIG-4 — 이카운트 주문서 마이그레이션")
public class EcountOrderImportController {

    private static final String PAGE_CODE = "ecount.mig4.order";
    private static final String ROLE_HEADER = "X-User-Role";

    private final EcountOrderImporter importer;
    private final DynamicPermissionClient dynamicPermissionClient;

    @PostMapping(value = "/ecount", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyRole('MASTER', 'MANAGER')")
    @Operation(summary = "이카운트 주문서 CSV staging 적재")
    ...
    private void checkEditPermission(String actorRole) {
        ...
        if (!dynamicPermissionClient.canEdit(actorRole, PAGE_CODE)
            ...
            throw new BusinessException(ErrorCode.FORBIDDEN,
                    "동적 권한 설정에 의해 주문서 import 권한이 차단되었습니다.");
        }
    }
}
