...
/** MIG-3 — Admin 이카운트 회계전표분개 CSV import. */
@Slf4j
@RestController
@RequestMapping("/admin/accounting/journal-entries/imports")
@RequiredArgsConstructor
@Tag(name = "MIG-3 — 이카운트 회계전표분개 마이그레이션")
public class EcountJournalEntryImportController {

    /** Codex BE M6 cycle 2 — MIG-3 회계전표분개 import 페이지 코드 (SP-D2 동적 권한). */
    private static final String PAGE_CODE = "accounting.ecount.mig3.journal-entry";
    private static final String ROLE_HEADER = "X-User-Role";

    private final EcountJournalEntryImporter importer;
    private final DynamicPermissionClient dynamicPermissionClient;

    @PostMapping(value = "/ecount", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyRole('MASTER', 'MANAGER')")
    @Operation(summary = "이카운트 회계전표분개 CSV 적재")
        ...
        if (!canEdit) {
            ...
            if (canView) {
                ...
                throw new BusinessException(ErrorCode.FORBIDDEN,
                        "동적 권한 설정에 의해 회계전표분개 import 권한이 차단되었습니다.");
            }
        }
    }
}
