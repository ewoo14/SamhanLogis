...
/**
 * 회계전표 협업 알림 수신자 식별자를 push 가능한 사용자 UUID 로 정규화한다.
 *
...
public class UserIdResolver {
    ...
    public Optional<UUID> resolve(String rawUserId) {
        ...
        if (SYSTEM_ACTOR_LITERALS.contains(normalized.toLowerCase(java.util.Locale.ROOT))) {
            log.debug("[JournalCollab] 시스템 actor 리터럴 skip — rawUserId={}", normalized);
            return Optional.empty();
        }
        try {
            UUID parsed = UUID.fromString(normalized);
            if (ZERO_UUID.equals(parsed)) {
                log.debug("[JournalCollab] zero-UUID actor skip — rawUserId={}", normalized);
                return Optional.empty();
            }
            ...
            if (resolved == null || resolved.isEmpty()) {
                log.debug("[JournalCollab] 알림 수신자 loginId resolve 실패 — loginId={}", normalized);
                return Optional.empty();
            }
            return resolved;
        }
