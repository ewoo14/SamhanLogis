 ...
 * 회계전표 협업 댓글.
 *
 * <p>shared/collab-core 의 {@link CollabCommentRecord} 를 ACCOUNTING_VOUCHER 문서에
 * 연결하는 concrete entity 다. 작성자 UUID 는 감사 추적 전용이며 사용자 화면에는 authorName 만
 * 표시한다.
 */
@Entity
@Table(name = "journal_collab_comments")
@SQLRestriction("is_deleted = false")
public class JournalCollabComment extends CollabCommentRecord {
    ...
    protected JournalCollabComment() {
    }

    private JournalCollabComment(CollabDocumentType documentType, UUID documentId, String anchor,
     ...
     * @return 영속화 전 신규 댓글
     */
    public static JournalCollabComment create(CollabDocumentType documentType, UUID documentId,
                                              String anchor, UUID authorId, String authorName,
                                              String body, UUID parentId) {
        return new JournalCollabComment(
