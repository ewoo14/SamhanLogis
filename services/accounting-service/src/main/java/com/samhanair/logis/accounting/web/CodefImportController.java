...
/** CODEF 은행·카드 거래내역 온디맨드 import endpoint. */
@RestController
@RequestMapping("/accounting/codef")
@RequiredArgsConstructor
@Tag(name = "CODEF 거래내역", description = "CODEF 은행·카드 거래내역 DRY_RUN/실연동 import")
public class CodefImportController {
    ...
    /** CODEF 은행·카드 거래내역을 조회해 BankTransaction 으로 적재한다. */
    @PostMapping("/import")
    @RequirePermission(page = PAGE_CODE, action = PermissionAction.CREATE)
    @Operation(summary = "CODEF 거래내역 import", description = "계좌/카드 ref 기준 온디맨드 조회 후 BankTransaction 적재")
