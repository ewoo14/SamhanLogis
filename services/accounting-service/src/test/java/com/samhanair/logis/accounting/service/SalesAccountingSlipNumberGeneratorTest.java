...
class SalesAccountingSlipNumberGeneratorTest {

    @Test
    void format_회계전표번호는_순번_선행0을_붙이지_않는다() {
        String slipNo = SalesAccountingSlipNumberGenerator.format(LocalDate.of(2026, 6, 8), 42);
    ...
    void next_회계전표번호_순번은_1이상이다() {
        String slipNo = new SalesAccountingSlipNumberGenerator().next(LocalDate.of(2026, 6, 8));
