...
/**
 * 회계전표 협업 알림 수신자 식별자 정규화 테스트.
 *
 * <p>분개 작성/게시 이력에는 UUID 문자열과 loginId(username)가 섞일 수 있다. 알림 발송 전
 * {@link UserIdResolver}가 UUID는 그대로 통과시키고, loginId는 auth-service 내부 조회 결과로
 * 변환하며, 시스템 actor는 원격 호출 없이 제외하는 계약을 고정한다.
 */
class UserIdResolverTest {
    ...
    /**
     * 시스템 audit 리터럴과 zero-UUID는 by-login 조회 없이 즉시 skip 한다.
     */
    @Test
