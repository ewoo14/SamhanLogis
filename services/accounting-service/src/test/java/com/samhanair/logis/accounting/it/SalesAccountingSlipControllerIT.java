...
class SalesAccountingSlipControllerIT extends AbstractPostgresIT {
    ...
    @BeforeEach
    void seedLockedSalesClosing() {
        requiresNewTransaction().executeWithoutResult(status -> {
            DailyClosing closing = DailyClosing.createV2(
                    LocalDate.of(2026, 5, 19), PARTNER_ID, DailyClosingKind.SALES,
    ...
    @AfterEach
    void removeSalesClosingFixture() {
        requiresNewTransaction().executeWithoutResult(status -> {
    ...
    @Test
    void POST_admin_sales_slips_일마감_미선행은_409로_차단한다() throws Exception {
        ...
        when(slipServiceClient.getSlipLine(sourceLineId)).thenReturn(new SlipLineSnapshot(
                sourceSlipId, "OUT-2026-05-0043", sourceLineId, PARTNER_ID,
    ...
    // ===== #850 R1 적대검증 보강 — HTTP 입력 계약(HIGH-2)·in-request 경계/누적(MED-1/3) =====
    ...
    void POST_admin_sales_slips_입력계약위반은_400_INVALID_INPUT(
            String label, CreateSalesAccountingSlipRequest req) throws Exception {
        mvc.perform(post("/admin/sales-slips")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("X-User-Id", "00000000-0000-0000-0000-000000000114")
                        .header("X-User-Role", "MASTER")
    ...
    private static CreateSalesAccountingSlipRequest invalidHttpRequest(BigDecimal allocQty, BigDecimal allocAmount) {
        return new CreateSalesAccountingSlipRequest(
                LocalDate.of(2026, 5, 19), PARTNER_ID, "P-2026-0001", "(주)한국공조",
                SalesTaxType.TAXABLE, "input contract",
                List.of(new CreateSalesAccountingSlipRequest.LineRequest(
                        "RX다배관", "RX다배관 30A", new BigDecimal("10"), new BigDecimal("150000"),
                        List.of(new CreateSalesAccountingSlipRequest.AllocationRequest(
                                UUID.randomUUID(), "OUT-2026-05-0042", UUID.randomUUID(), 1,
     ...
     */
    @Test
    void POST_admin_sales_slips_in_request_동일원천_두배분_합계_잔여경계_통과() throws Exception {
        UUID sourceSlipId = UUID.randomUUID();
        UUID sourceLineId = UUID.randomUUID();
        ...
        assertThat(salesSlipRepository.findBySlipNo("2026/05/19-INBND")).isPresent();
    }
     ...
     */
    @Test
    void POST_admin_sales_slips_in_request_누적초과는_422_이고_전표_미저장() throws Exception {
        UUID sourceSlipId = UUID.randomUUID();
        UUID sourceLineId = UUID.randomUUID();
        ...
        assertThat(salesSlipRepository.findBySlipNo("2026/05/19-INOVER")).isEmpty();
        assertThat(salesSlipRepository.count()).isEqualTo(before);
    }
}
