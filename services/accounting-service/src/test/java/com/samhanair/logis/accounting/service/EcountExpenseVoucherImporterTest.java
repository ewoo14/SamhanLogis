...
/** MIG-5 지출결의서 importer behavior 회귀 가드. */
@ExtendWith(MockitoExtension.class)
class EcountExpenseVoucherImporterTest {

    @Mock private NamedParameterJdbcTemplate jdbcTemplate;
    @Mock private PartnerLookupClient partnerLookupClient;

    private EcountExpenseVoucherImporter importer;
    ...
    @Test
    void staging_적재_정상() {
        EcountMig5ImportResult result = importer.importCsv(stream(csv(row("2026/05/18 -247", "지출결의서", "2,380,000", "거래처A"))), "tester");
    ...
    void MIG5_TRANSACTION_TYPE_INVALID() {
        EcountMig5ImportResult result = importer.importCsv(stream(csv(row("2026/05/18 -247", "입금보고서", "1,000", "거래처A"))), "tester");
    ...
    void MIG5_LOOKUP_MISS() {
        ...
        EcountMig5ImportResult result = importer.importCsv(stream(csv(row("2026/05/18 -247", "지출결의서", "1,000", "미등록"))), "tester");
    ...
    void MIG5_AMOUNT_INVALID() {
        EcountMig5ImportResult result = importer.importCsv(stream(csv(row("2026/05/18 -247", "지출결의서", "-1", "거래처A"))), "tester");
    ...
    private static PartnerSummary partner() {
        return new PartnerSummary(UUID.fromString("dddddddd-dddd-dddd-dddd-dddddddddddd"),
