...
/**
 * 동적 RBAC 권한 조회 클라이언트 인터페이스 — SP-D3 notification-service 이식.
 *
 * <p>auth-service 의 {@code GET /auth/admin/permissions/check} endpoint 를 호출하여
 * 특정 역할의 특정 페이지 접근 가능 여부를 확인한다.
 *
 * <p>MSA 패턴: notification-service 는 자체 DB 에 권한 정보를 갖지 않으므로
...
public interface DynamicPermissionClient {
     ...
     *
     * @param roleCode 역할 코드 (예: DISPATCH)
     * @param pageCode 페이지 코드 (예: notification.dispatch-sms.send-audit)
