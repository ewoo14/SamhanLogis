...
/** MIG-6 — Admin 이카운트 사원 CSV import. */
@Slf4j
@RestController
@RequestMapping("/admin/user/employees/imports")
@RequiredArgsConstructor
@Tag(name = "MIG-6 — 이카운트 사원 마이그레이션")
public class EcountEmployeeImportController {

    private static final String PAGE_CODE = "ecount.mig6.employee";
    private final EcountEmployeeImporter importer;
    private final DynamicPermissionClient dynamicPermissionClient;

    @PostMapping(value = "/ecount", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyRole('MASTER', 'MANAGER')")
    @Operation(summary = "이카운트 사원 CSV 적재")
    ...
    private void checkEditPermission(String actorRole) {
        ...
        if (!dynamicPermissionClient.canEdit(actorRole, PAGE_CODE)
            ...
            throw new BusinessException(ErrorCode.FORBIDDEN,
                    "동적 권한 설정에 의해 사원 import 권한이 차단되었습니다.");
        }
    }
}
