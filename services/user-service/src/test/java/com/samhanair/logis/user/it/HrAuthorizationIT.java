...
class HrAuthorizationIT extends AbstractPostgresIT {
     ...
     */
    @Test
    @DisplayName("TC-1: 대표실+MASTER → /admin/users 가드 통과 (status != 403)")
    void tc1_executiveOfficeMaster_allowed() throws Exception {
        // 가드 통과 시 status 가 가드 거절 (403) 이 아님을 검증.
        // 정확한 200 검증은 EmployeeRepository.searchAdmin 응답 schema + DB 시드 의존이 커서
        // 후속 슬라이스에서 보강. 본 TC 는 @hr.isExecutiveOffice() and hasAnyRole('MASTER','MANAGER') SpEL
        // 평가 통과만 단언.
        ...
        org.assertj.core.api.Assertions.assertThat(status)
                .as("대표실+MASTER 가드 통과 검증 (실제 status=%d)", status)
                .isNotEqualTo(403);
    }
     ...
     */
    @Test
    @DisplayName("TC-5: X-User-Department 헤더 없는 MASTER → /admin/users (legacy fallback 정책 — 가드 통과)")
    void tc5_missingDepartmentHeader_legacyFallbackAllowed() throws Exception {
        // PR #160 회귀 가드 — HrAuthorizationHelper 가 X-User-Department 부재 시
        // X-User-Role MASTER/MANAGER 일 때 임시 허용 (legacy 토큰 호환).
        // 전체 사용자 재로그인 완료 후 fallback 제거 시점에 본 TC 를 expects(403) 으로 복원.
        var result = mockMvc.perform(get("/api/v1/admin/users")
                        .header("X-User-Id", "a0000000-0000-0000-0000-000000000001")
                        .header("X-User-Role", "MASTER"))
                .andReturn();
        int status = result.getResponse().getStatus();
        org.assertj.core.api.Assertions.assertThat(status)
                .as("legacy fallback 가드 통과 검증 (실제 status=%d)", status)
                .isNotEqualTo(403);
    }
}
