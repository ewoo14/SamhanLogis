 ...
 * 배차 협업 수정 이력.
 *
 * <p>changeSet 저장/상태 전이는 collab-core base 가 소유한다. 본 entity 는 배차 도메인 테이블,
 * UUID PK, 수정완료 동시 결정 방지를 위한 optimistic lock 버전만 제공한다.
 */
@Entity
@Table(name = "dispatch_collab_suggestions")
@SQLRestriction("is_deleted = false")
public class DispatchCollabSuggestion extends CollabSuggestionRecord {
    ...
    protected DispatchCollabSuggestion() {
    }

    private DispatchCollabSuggestion(CollabDocumentType documentType, UUID documentId,
     ...
     * 신규 배차 수정 이력 factory.
     *
     * @param documentType DISPATCH_TASK
     ...
     * @return 영속화 전 신규 제안
     */
    public static DispatchCollabSuggestion create(CollabDocumentType documentType, UUID documentId,
                                                  UUID proposerId, String proposerName,
                                                  String changeSet, String reason) {
        return new DispatchCollabSuggestion(
