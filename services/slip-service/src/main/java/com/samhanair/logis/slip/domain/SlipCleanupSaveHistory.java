 ...
 * 전표정리 결과 저장내역.
 *
 * <p>legacy GAS 전표정리리스트 조회 조건과 결과를 PostgreSQL JSONB 로 저장한다.
 * BaseEntity 7 audit 필드와 {@code is_deleted=false} soft-delete restriction 을 사용한다.
 */
@Entity
@Getter
@Table(name = "slip_cleanup_save_history")
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@SQLRestriction("is_deleted = false")
public class SlipCleanupSaveHistory extends BaseEntity {
    ...
    private SlipCleanupProgramType programType;

    @Enumerated(EnumType.STRING)
    @Column(name = "save_mode", nullable = false, length = 20)
    private SlipCleanupSaveMode saveMode;
    ...
    private SlipCleanupSaveHistory(SlipCleanupProgramType programType,
     ...
     */
    public static SlipCleanupSaveHistory create(SlipCleanupProgramType programType,
                                                SlipCleanupSaveMode saveMode,
                                                String topic,
                                                JsonNode requestParams,
                                                JsonNode responsePayload) {
        ...
        return new SlipCleanupSaveHistory(programType, saveMode, topic, requestParams, responsePayload);
    }
     ...
     */
    public SlipCleanupSaveHistory supersedeBy(String userId) {
        markDeleted(userId);
        return this;
    }

    private static String normalizeTopic(String topic, SlipCleanupSaveMode saveMode) {
        if (topic == null || topic.isBlank()) {
            return saveMode == SlipCleanupSaveMode.AUTO_LATEST ? "자동저장" : "";
        }
        return topic.trim();
    }
}
