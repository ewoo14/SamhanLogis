...
public class DispatchSaveHistoryService {
    ...
    public Page<DispatchSaveHistoryListRow> list(
        ...
        return repository.findByFilter(
                        ...
                        range.toExclusive(),
                        pageable)
                .map(DispatchSaveHistoryListRow::from);
    }
     ...
     */
    @Transactional(readOnly = true)
    public DispatchSaveHistoryDetailResponse findDetail(UUID id, String currentUser) {
        String user = normalizeUser(currentUser);
     ...
     */
    @Transactional(readOnly = true)
    public DispatchSaveHistoryDetailResponse findLatestAutoLatest(
            DispatchProgramType programType,
            String currentUser) {
        if (programType == null) {
    ...
    private void validateRequest(DispatchSaveHistoryRequest request) {
        ...
        if (request.programType() == null || request.saveMode() == null
            ...
            throw new BusinessException(ErrorCode.INVALID_INPUT, "배차 저장내역 필수값이 누락되었습니다.");
        }
        if (request.saveMode() == DispatchSaveMode.MANUAL_NAMED
    ...
    private int payloadSize(Object payload) {
        try {
            ...
            throw new BusinessException(ErrorCode.INVALID_INPUT,
                    "배차 저장 payload 를 직렬화할 수 없습니다.", ex);
        }
    }
