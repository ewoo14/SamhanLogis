...
-- 혼합 주문은 실측 0건이며, 방어적으로 활성 정본 라인이 남으면 부모를 보존한다.
...
UPDATE partner_order_lines l SET is_deleted=TRUE, deleted_at=COALESCE(l.deleted_at,CURRENT_TIMESTAMP),
...
UPDATE partner_orders o SET is_deleted=TRUE, deleted_at=COALESCE(o.deleted_at,CURRENT_TIMESTAMP),
 ...
 AND NOT EXISTS (SELECT 1 FROM partner_order_lines l WHERE l.partner_order_id=o.id AND l.is_deleted=FALSE);

-- 정확한 복구:
-- UPDATE partner_order_lines SET is_deleted=FALSE,deleted_at=NULL,deleted_by=NULL WHERE deleted_by='issue-1096-test-seed-cleanup';
-- UPDATE partner_orders SET is_deleted=FALSE,deleted_at=NULL,deleted_by=NULL,deleted_by_name=NULL WHERE deleted_by='issue-1096-test-seed-cleanup';
