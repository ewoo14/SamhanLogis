...
public interface PriceChangeScheduleRepository extends JpaRepository<PriceChangeSchedule, UUID> {
    ...
    /** 활성 스케줄 전체를 category 오름차순으로 조회한다. */
    List<PriceChangeSchedule> findByIsDeletedFalseOrderByCategoryAsc();

    /** categoryKey 기준 활성 스케줄을 조회한다. */
    Optional<PriceChangeSchedule> findByCategoryAndIsDeletedFalse(String category);
}
