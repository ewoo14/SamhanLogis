-- #1051 계열 보완: V31의 products 광역 soft-delete 복구.
--
-- V31은 이미 적용된 migration이므로 수정하지 않는다. V31이 의도한 products 및
-- 자식 행의 범위는 이 파일에 재현한 101개 UUID로 고정한다.
-- V31 적용 후 deleted_by actor가 붙은 비대상 행만 복구하고, 복구된 비상품 후보는
...
-- V31 actor가 표시한 제품 중 명시된 101개가 아닌 것만 복구한다.
UPDATE products p
   SET is_deleted = FALSE,
       deleted_at = NULL,
       deleted_by = NULL
 WHERE p.is_deleted = TRUE
   AND p.deleted_by = 'issue-1096-test-seed-cleanup'
   AND NOT EXISTS (
       SELECT 1 FROM _issue_1096_test_product_ids ids WHERE ids.id = p.id
   );
...
UPDATE product_aliases a
 ...
 WHERE a.is_deleted = TRUE
   AND a.deleted_by = 'issue-1096-test-seed-cleanup'
   AND NOT EXISTS (
       SELECT 1 FROM _issue_1096_test_product_ids ids WHERE ids.id = a.main_product_id
   );

UPDATE bundle_component b
   SET is_deleted = FALSE,
       deleted_at = NULL,
       deleted_by = NULL
 WHERE b.is_deleted = TRUE
   AND b.deleted_by = 'issue-1096-test-seed-cleanup'
   AND NOT EXISTS (
       SELECT 1 FROM _issue_1096_test_product_ids ids WHERE ids.id = b.bundle_product_id
   );
...
UPDATE product_estimate_exposure e
 ...
 WHERE e.is_deleted = TRUE
   AND e.deleted_by = 'issue-1096-test-seed-cleanup'
   AND NOT EXISTS (
       SELECT 1 FROM _issue_1096_test_product_ids ids WHERE ids.id = e.product_id
   );
