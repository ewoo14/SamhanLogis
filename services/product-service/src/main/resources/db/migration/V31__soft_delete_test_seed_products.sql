...
-- product_spec는 실측상 active 참조 0건, price_history도 0건이라 변경하지 않는다.
-- inventory_db의 재고/이력은 inventory-service 소유라 이 migration에서 변경하지 않는다.
...
UPDATE product_aliases a SET is_deleted=TRUE, deleted_at=COALESCE(a.deleted_at,CURRENT_TIMESTAMP),
 deleted_by='issue-1096-test-seed-cleanup'
 WHERE a.is_deleted=FALSE AND a.main_product_id IN (SELECT id FROM _issue_1096_test_product_ids);

UPDATE bundle_component b SET is_deleted=TRUE, deleted_at=COALESCE(b.deleted_at,CURRENT_TIMESTAMP),
 deleted_by='issue-1096-test-seed-cleanup'
 WHERE b.is_deleted=FALSE AND b.bundle_product_id IN (SELECT id FROM _issue_1096_test_product_ids);

UPDATE product_estimate_exposure e SET is_deleted=TRUE, deleted_at=COALESCE(e.deleted_at,CURRENT_TIMESTAMP),
 deleted_by='issue-1096-test-seed-cleanup'
 WHERE e.is_deleted=FALSE AND e.product_id IN (SELECT id FROM _issue_1096_test_product_ids);

UPDATE products p SET is_deleted=TRUE, deleted_at=COALESCE(p.deleted_at,CURRENT_TIMESTAMP),
 deleted_by='issue-1096-test-seed-cleanup'
 WHERE p.is_deleted=FALSE AND p.created_by IN ('system','qa-seed');

-- 정확한 복구:
-- UPDATE products SET is_deleted=FALSE,deleted_at=NULL,deleted_by=NULL WHERE deleted_by='issue-1096-test-seed-cleanup';
-- UPDATE product_aliases SET is_deleted=FALSE,deleted_at=NULL,deleted_by=NULL WHERE deleted_by='issue-1096-test-seed-cleanup';
-- UPDATE bundle_component SET is_deleted=FALSE,deleted_at=NULL,deleted_by=NULL WHERE deleted_by='issue-1096-test-seed-cleanup';
-- UPDATE product_estimate_exposure SET is_deleted=FALSE,deleted_at=NULL,deleted_by=NULL WHERE deleted_by='issue-1096-test-seed-cleanup';
