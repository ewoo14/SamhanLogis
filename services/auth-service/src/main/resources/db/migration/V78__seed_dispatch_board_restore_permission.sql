-- E2 pillar2 dispatch strikethrough restore.
--
-- dispatch.board existed before the 7-action permission split and had UPDATE/DELETE
-- semantics for MASTER/MANAGER/DISPATCH, but no explicit RESTORE grant. Restore changes
-- board state, so grant it to the same operational roles and rematerialize account cache.
...
SET can_restore = TRUE,
    modified_at = NOW(),
    modified_by = 'v78-dispatch-board-restore';
...
JOIN group_page_permissions gpp
 ...
 AND gpp.page_code = 'dispatch.board'
