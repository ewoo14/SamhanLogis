-- V69__seed_dispatch_external_carriers_page_codes.sql
-- 외부기사/배송사 마스터 page-code: dispatch.external-carriers.
--
-- slip-service account-mode enforcement 는 account_page_permissions.can_<action> 을 조회한다.
-- 따라서 V66 패턴과 동일하게 role/template/group/account 4-table 을 함께 동기화한다.
...
SELECT
    ...
    TRUE,
    NOW(),
    'v69-dispatch-external-carriers',
    NOW(),
    'v69-dispatch-external-carriers',
    FALSE
FROM (VALUES
...
SET can_view = TRUE,
    can_edit = TRUE,
    modified_at = NOW(),
    modified_by = 'v69-dispatch-external-carriers';
...
JOIN group_page_permissions gpp
 ...
 AND gpp.page_code = 'dispatch.external-carriers'
