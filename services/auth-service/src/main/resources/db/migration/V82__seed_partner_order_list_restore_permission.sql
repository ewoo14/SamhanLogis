-- E2 partner order list inline restore permission.
--
-- sales.partner-order.list existed before soft-deleted rows were visible on the
-- list page. Restore is an operational list action, so grant can_restore to the
-- same internal roles that manage partner orders and rematerialize account cache.
...
JOIN group_page_permissions gpp
 ...
 AND gpp.page_code = 'sales.partner-order.list'
