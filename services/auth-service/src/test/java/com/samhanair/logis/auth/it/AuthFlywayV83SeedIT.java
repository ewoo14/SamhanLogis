...
class AuthFlywayV83SeedIT extends AbstractPostgresIT {

    private static final String PAGE_CODE = "sales.partner-order.list";

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Test
    @DisplayName("V83은 MASTER/MANAGER/SALES 역할 템플릿에 sales.partner-order.list can_restore를 seed한다")
    void threeRoleTemplatesHaveRestore() {
        // V83 ON CONFLICT DO UPDATE 는 can_restore 만 보장한다 — 선행 시드(V10 계열)가 만든
        // 기존 행의 view/update 값은 V83 관할 밖이라 단언하지 않는다(false-RED 방지).
    ...
    @Test
    @DisplayName("V83은 빌트인 그룹 100/101/102에 sales.partner-order.list can_restore를 seed한다")
    ...
    @Test
    @DisplayName("V83은 sales.partner-order.list RESTORE 계정 캐시를 materialize하고 시스템 마스터는 bypass로 제외한다")
