...
function getFilterWordsFromNotion() {
  var payload = {
    filter: { property: '프로그램유형', select: { equals: '거래원장(필터링단어)' } },
...
function getClientCodesFromNotion() {
  var payload = {
    filter: { property: '프로그램유형', select: { equals: '거래원장(거래처코드)' } },
    sorts: [{ timestamp: 'created_time', direction: 'descending' }],
    page_size: 1
  };
  var opts = {
...
function getHistoryFromNotion(start, end) {
  ...
  var payload = {
    filter: {
      and: [
        { property: '프로그램유형', select: { equals: '거래원장결과' } },
...
function getLatestHistoryFromNotion() {
  Logger.log('🔄최신조회');
  var payload = {
    filter: { property: '프로그램유형', select: { equals: '거래원장결과' } },
    sorts: [{ timestamp: 'created_time', direction: 'descending' }],
    page_size: 1
  };
