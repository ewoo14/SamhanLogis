...
function getHistoryFromNotion(start, end) {
  ...
  var payload = {
    filter: {
      and: [
        { property: '프로그램유형', select: { equals: '교차검증' } },
  ...
  try {
    ...
    return data.results.map(row => {
      ...
      }; // 목록에서는 데이터 용량을 줄이기 위해 압축 텍스트 전송을 생략합니다.
    });
  } catch(e) { return []; }
}

// 특정 ID의 상세 내역 조회 (수동 복원용)
function getSpecificHistory(pageId) {
  ...
  try {
    ...
    return decompressString(b64Str); // 압축 해제 후 JSON 문자열 반환
  } catch(e) { return null; }
}

// 최신 단건 조회 (접속 시 자동 복원용)
function getLatestHistoryFromNotion() {
  Logger.log('🔄최신조회');
  var payload = {
    filter: { property: '프로그램유형', select: { equals: '교차검증' } },
    sorts: [{ timestamp: 'created_time', direction: 'descending' }],
    page_size: 1
  };
  ...
  try {
    ...
    return { data: decompressString(b64Str) }; // 백엔드에서 압축 해제 후 전송
  } catch(e) { return null; }
}