...
const NOTION_TOKEN_MANUAL = 'REDACTED_NOTION_TOKEN';
const NOTION_DB_ID_MANUAL = '337a1006d65880a8b633fe6ca44573b2';

// 렌더링
function doGet() {
  Logger.log('🚀 시작');
  return HtmlService.createHtmlOutputFromFile('Index')
    .setTitle('삼한공조시스템 배차현황리스트')
...
function getUserAuth() {
  ...
  try {
    ...
    if (res.getResponseCode() !== 200) {
      Logger.log('🛑 거부');
      return { authorized: false, email: email, error: '노션 접근 거부 (DB 연결 또는 속성명 확인)' };
    }
...
function getHistoryFromNotion(sDate, eDate) {
  var payload = {
    filter: {
      and: [
        { property: '프로그램유형', select: { equals: '미배차리스트' } },
...
function getLatestHistoryFromNotion() {
  ...
  try {
    var payload = {
      filter: { property: '프로그램유형', select: { equals: '미배차리스트' } },
      sorts: [{ timestamp: 'created_time', direction: 'descending' }],
      page_size: 1
    };
...
// 수동
function saveManualDataToNotion(dataStr, email, name, programType, dateStr) {
  Logger.log('💾 수동');
  var max = 2000;
  var arr1 = [];
  var arr2 = [];
  ...
  // 프론트에서 전달받은 날짜 사용
  var topicName = '수동저장(' + dateStr + ')';

  var payload = {
    parent: { database_id: NOTION_DB_ID_MANUAL },
    properties: {
      '작업자': { title: [{ text: { content: name } }] },
      '작업계정': { email: email },
      '프로그램유형': { select: { name: programType } },
      '저장주제': { rich_text: [{ text: { content: topicName } }] },
      '저장내역1': { rich_text: arr1 },
      '저장내역2': { rich_text: arr2 }
    }
  };
  ...
  var options = {
    method: 'post',
    headers: {
      'Authorization': 'Bearer ' + NOTION_TOKEN_MANUAL,
      'Notion-Version': '2022-06-28',
      'Content-Type': 'application/json'
    },
    payload: JSON.stringify(payload)
