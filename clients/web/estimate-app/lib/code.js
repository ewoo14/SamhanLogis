    ...
    merged.forEach((it) => {
      ...
      SaleList.push({
        BulkDatas: {
          ...
          PROD_DES: String(it.name || it.productName || ''),
          CATEGORY_KEY: String(it.categoryKey || ({ HOME: 'homemulti', COMM: 'commercialMulti', SINGLE: 'singleSets', OLD: 'oldProducts' }[it.section] || '')),
          BUNDLE_SET_OPTIONS: it.bundleSetOptions || it.setOptions || null,
          SIZE_DES: sizeDes,
          QTY: String(qty),
...
async function getQuoteHistory(startDate, endDate) {
  ...
  if (resp.status < 200 || resp.status >= 300) {
    const error = new Error(`snapshot history failed: HTTP ${resp.status}`);
    error.statusCode = resp.status;
    error.responseData = resp.data;
    throw error;
  }
...
async function getQuoteHistoryByCustomer(custName) {
  ...
  if (resp.status < 200 || resp.status >= 300) {
    const error = new Error(`snapshot customer history failed: HTTP ${resp.status}`);
    error.statusCode = resp.status;
    error.responseData = resp.data;
    throw error;
  }
