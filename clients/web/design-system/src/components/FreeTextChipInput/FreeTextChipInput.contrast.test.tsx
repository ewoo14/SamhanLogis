...
/**
 * FreeTextChipInput 대비 계약.
 *
 * <p>이 컴포넌트의 칩은 value-only TagChip 이다. 과거 테스트는 spacing 토큰 존재만 확인하고
 * 실제 칩 색(전경/배경)은 검사하지 않아 허위였다(AsyncAutocomplete.contrast 패턴 미준수).
 * 여기서는 TagChip.module.css 의 실제 색 토큰을 tokens.css 실효값(light `:root` + dark 오버라이드)
 * 으로 해석해 WCAG 대비를 실측한다 — value(본문 텍스트) ≥ 4.5:1, remove(X 버튼 그래픽) ≥ 3:1.
 *
 * <p>[[feedback_css_var_token_not_fallback]] — 토큰이 정의되면 토큰값이 렌더되므로 fallback
 * 리터럴이 아니라 tokens.css 실효값을 재계산한다. 어느 토큰이든 대비를 깨는 값으로 바뀌면 RED.
 */
...
describe('FreeTextChipInput 대비 계약', () => {
  ...
  it.each(['light', 'dark'] as const)(
    '%s 테마에서 value-only 칩 전경/배경 대비가 WCAG 기준을 충족한다',
    (theme) => {
      const bg = token(theme, chipBgToken)
      const value = token(theme, valueToken)
      const remove = token(theme, removeToken)
      if (!bg || !value || !remove) throw new Error(`${theme} 테마 토큰 미해석`)

      // value = 칩 본문 텍스트(13px) → small text 4.5:1
