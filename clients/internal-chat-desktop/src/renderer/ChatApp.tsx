...
import { createDirectChatRoom, fetchChatMessages, fetchChatRooms, searchRecipients, sendChatMessage, subscribeToChatRoom, type RecipientOption } from './api/chatApi'
...
function ChatRoomsPage() {
  ...
  return <main className="chat-layout" data-testid="chat-rooms-page"><header className="chat-header"><h1>채팅</h1><Button type="button" onClick={() => document.getElementById('chat-new-conversation')?.focus()}>새 대화</Button></header><Card><section aria-label="새 대화" id="chat-new-conversation" tabIndex={-1}><Input aria-label="대화 상대 검색" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이름 검색" />{recipients.data?.map((recipient) => <button className="recipient" type="button" key={recipient.userId} onClick={() => setSelected(recipient)}>{recipient.name} · {recipient.department ?? ''} · {recipient.employeeCode ?? ''}</button>)}{selected ? <div className="selected-recipient"><span>{selected.name}</span><Button type="button" onClick={() => create.mutate()} disabled={create.isPending}>대화 시작</Button></div> : null}{create.isError ? <p role="alert">대화방을 만들지 못했습니다. 잠시 후 다시 시도해 주세요.</p> : null}</section></Card><Card><ul aria-label="채팅방 목록" className="room-list">{rooms.data?.map((room) => <li key={room.roomCode}><Link to={`/chat/${encodeURIComponent(room.roomCode)}`}>{roomLabel(room)}</Link></li>)}</ul></Card></main>
}
