...
function ChatRoomsPage() {
  ...
  const create = useMutation({ mutationFn: () => createDirectChatRoom(selected!.userId), onSuccess: (room) => { void queryClient.invalidateQueries({ queryKey: ['chat', 'rooms'] }); navigate(encodeURIComponent(room.roomCode)) } })
