...
const observed = { direct: 0, leave: 0 }
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname.startsWith('/api/users/messenger/presence/sessions/') && req.method === 'POST') return json(res, null)
...
async function main() {
  ...
  await page.waitForSelector('textarea[aria-label="메시지 본문"]')
  assert.equal(observed.direct, 1)
  await page.screenshot({ path: path.join(screenshotDir, 'round-fix-2-direct-real-electron.png'), fullPage: true })
  ...
  console.log(`ELECTRON_CONTRACT|direct=${observed.direct}|group=metadata|presence=reflected|leave=${observed.leave}`)
}
