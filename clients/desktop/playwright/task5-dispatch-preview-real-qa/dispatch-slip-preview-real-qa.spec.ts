...
/**
 * task5 배차 전표확인 = 판매전표 미리보기 — 실서버 GUI QA.
 *
 * 배차보드 미배차 전표 "전표확인" → SlipDetailModal 이 DispatchDocument(판매전표 문서)를
...
test('task5 배차 전표확인 → 판매전표 미리보기 모달(xl·무잘림)', async ({ browser }) => {
  ...
  // 판매전표 문서 본문(DispatchDocument .dispatch-page) 렌더 확인.
  ...
  expect(docBox, '판매전표 문서 bounding box').not.toBeNull()
  if (docBox) {
