...
const ORDER_BASE = process.env['R16_ORDER_BASE_URL'] ?? 'http://127.0.0.1:5186'
const DESKTOP_BASE = process.env['R16_DESKTOP_BASE_URL'] ?? 'http://127.0.0.1:5176'
const PARTNER_API = process.env['R16_PARTNER_API'] ?? 'http://127.0.0.1:28188'
const PRODUCT_API = process.env['R16_PRODUCT_API'] ?? 'http://127.0.0.1:28184'
const GATEWAY_BASE = process.env['R16_GATEWAY_BASE'] ?? 'http://127.0.0.1:8080'
...
const SHOTS = resolveQaShotsDir(path.resolve(HERE, '../../../../docs/qa/1241-r16-adversarial/screenshots'))
const RESULTS = path.join(SHOTS, 'results.json')

const identityHeaders = {
  'X-Samhan-Gateway-Attestation': ATTESTATION,
  'X-User-Id': USER_ID,
  'X-User-Name': encodeURIComponent('SOL R16 적대검증'),
  'X-Is-Partner': 'true',
...
test('세트 전수 실화면 — 품목표·미리보기·최종확인·격리 저장', async ({ page }) => {
  ...
  await page.locator('#addrBase').evaluate((node: HTMLInputElement) => {
    node.value = '서울특별시 R16 격리 QA로 16'
  ...
  await page.locator('#memo').fill('R16 헤더 보존 격리 QA')
...
test('가격 미리보기 500 미재발 + VAT 4경계 실HTTP·격리 저장', async ({ request }) => {
  ...
  for (const amount of [5, 6, 11, 800000]) {
    const lines = [{ modelCode: 'AR06D1150HZN', categoryKey: 'singleSets', quantity: 1, unitPrice: amount, setAllocation: true, remark: `R16 VAT ${amount}` }]
    ...
    const order = {
      ...
      addr: `서울특별시 R16 VAT ${amount}`,
      auditAddr: `서울특별시 R16 VAT ${amount}`,
      tel: '01016161616',
      due: '2026-08-20',
      payDue: '2026-08-31',
      memo: `R16 VAT 경계 ${amount}`,
    }
    ...
    const draft = await request.post(`${PARTNER_API}/api/v1/partner-orders/drafts`, {
      headers: identityHeaders,
      data: { label: `R16 VAT ${amount}`, payloadJson: JSON.stringify({ items, order }) },
    })
...
test('데스크톱 실화면 — R03·R05 저장 상세과 시트 폐기 전달', async ({ page }) => {
  ...
  await page.route(`${GATEWAY_BASE}/auth/admin/permissions/my`, route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, code: 'OK', message: '성공', data: { 'sales.partner-order.list': ['VIEW'], 'products.sync': ['VIEW', 'CREATE'] } }) }))
  ...
  const savedAddresses = page.getByText('서울특별시 R16 격리 QA로 16 16층', { exact: false })
  ...
  await expect(page.getByText('R16 헤더 보존 격리 QA', { exact: true })).toBeVisible()
  console.log(`R03_R05_SCREEN orderNo=${qaResults.mainOrderNo} partner=주식회사 중앙유통 address=서울특별시 R16 격리 QA로 16 16층 paymentDue=2026-08-31 memo=R16 헤더 보존 격리 QA`)
  ...
  await page.screenshot({ path: path.join(SHOTS, '07-sheet-sync-retired-real-qa.png'), fullPage: true })
