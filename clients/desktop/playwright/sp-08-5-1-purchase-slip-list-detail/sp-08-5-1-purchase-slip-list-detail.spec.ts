...
test.describe('SP-08-5-1 매입 목록/상세 계약', () => {
  test('T1 slip-service R1/R2 매입 endpoint contract', () => {
    ...
    expect(detailDto).not.toContain('UUID id,')
    expect(detailDto).not.toContain('UUID partnerId,')
    expect(detailDto).not.toContain('UUID sourceWarehouseId,')
    expect(detailDto).not.toContain('UUID destinationWarehouseId,')
    expect(detailDto).not.toContain('UUID deliveryBatchId,')
  ...
  test('T3 권한 가드는 WAREHOUSE/MANAGER/MASTER만 허용하고 INVENTORY를 제외한다', () => {
    ...
    expect(controller).toContain('"WAREHOUSE".equals(role) || "MANAGER".equals(role) || "MASTER".equals(role)')
    expect(controller).toContain('매입 전표 조회는 WAREHOUSE / MANAGER / MASTER 권한만 허용됩니다')
  ...
  test('T5 SP-03 회귀: SAVED/CONFIRMED 검수 CTA가 유지된다', () => {
    ...
    expect(decisions).toContain('WAREHOUSE / MANAGER / MASTER')
  })
})
