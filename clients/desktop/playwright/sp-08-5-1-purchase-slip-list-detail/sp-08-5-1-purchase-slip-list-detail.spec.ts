...
test.describe('SP-08-5-1 매입 목록/상세 계약', () => {
  test('T1 slip-service R1/R2 매입 endpoint contract', () => {
    ...
    expect(detailDto).toContain('String inspectionStatus')
    expect(detailDto).toContain('case SAVED, CONFIRMED -> "READY"')
  ...
  test('T3 권한 가드는 WAREHOUSE/MANAGER/MASTER만 허용하고 INVENTORY를 제외한다', () => {
    ...
    expect(it).toContain('testListInboundForbiddenForInventory')
  })
  ...
  test('T5 SP-03 회귀: SAVED/CONFIRMED 검수 CTA가 유지된다', () => {
    ...
    expect(dialog).toContain("invalidateQueries({ queryKey: ['slips', 'list', 'INBOUND'] })")
