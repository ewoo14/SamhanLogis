...
test.describe('menu-5category view-only mutation gates', () => {
  ...
  test('배차 SMS view-only: 미리보기 버튼이 비활성화된다', async ({ page }) => {
    await gotoWithPerm(page, '/arologis/dispatch-sms', 'notification.dispatch-sms.send-audit', false, 'DISPATCH')
  ...
  test('배차 SMS create 보유: 미리보기 버튼이 활성화된다', async ({ page }) => {
    await gotoWithPerm(page, '/arologis/dispatch-sms', 'notification.dispatch-sms.send-audit', true, 'DISPATCH')
