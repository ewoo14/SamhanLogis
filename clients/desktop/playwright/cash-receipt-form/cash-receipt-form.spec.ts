...
const DRAFT_RECEIPT_ID = '00000000-0000-4000-8000-000000000709'

async function openDraftReceipt(page: Page): Promise<string> {
  await page.goto(`${BASE_URL}/#/accounting/admin/cash-receipts/${DRAFT_RECEIPT_ID}?mockRole=MASTER`, { waitUntil: 'domcontentloaded' })
  await expect(page).toHaveURL(new RegExp(`/accounting/admin/cash-receipts/${DRAFT_RECEIPT_ID}`))
  return `${BASE_URL}/#/accounting/admin/cash-receipts/${DRAFT_RECEIPT_ID}`
}

test.describe('입금보고서 조회폼 + 상세/편집 (E3 S4b)', () => {
  test.use({ viewport: { width: 1440, height: 900 } })

  test('조회: 신규 작성 경로는 목록으로 redirect되고 목록에 신규 작성 버튼이 없다', async ({ page }) => {
    await page.goto(`${BASE_URL}/#/accounting/admin/cash-receipts/new?mockRole=MASTER`, { waitUntil: 'domcontentloaded' })

    // A: 직접 작성 URL은 작성폼이 아니라 조회 목록으로 안전하게 되돌린다.
    ...
    // B: 목록에서 수기 신규 작성 CTA를 제공하지 않는다.
    await expect(page.getByRole('button', { name: '신규 작성' })).toHaveCount(0)
  })

  test('상세: 필드·목록 복귀·DRAFT 액션 노출', async ({ page }) => {
    await openDraftReceipt(page)

    await expect(page.locator('h3').filter({ hasText: /\d{4}\/\d{2}\/\d{2}-\d+/ })).toBeVisible()
    await expect(page.getByText('미래유통')).toBeVisible()
  ...
  test('편집: DRAFT hydrate 후 PATCH 저장', async ({ page }) => {
    const detailUrl = await openDraftReceipt(page)
    ...
    await expect(page.getByLabel('거래처명')).toHaveValue('미래유통')
