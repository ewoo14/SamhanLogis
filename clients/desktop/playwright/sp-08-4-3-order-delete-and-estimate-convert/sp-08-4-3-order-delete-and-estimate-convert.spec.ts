...
test.describe('SP-08-4-3 주문 삭제 + 견적 주문 변환 정적 계약', () => {
  test('T1: BE DELETE endpoint contract', () => {
    ...
    // E2 소급: deleted_by_name 보존을 위해 softDeleteCascadeWithName + resolveActorName(actorId, actorName) 로 변경됨.
    expect(service).toContain('softDeleteCascadeWithName(')
    expect(service).toContain('resolveActorName(actorId, actorName)')
  ...
  test('T2: BE from-estimate endpoint contract', () => {
    ...
    expect(service).toContain('pg_advisory_xact_lock(CAST(hashtext(?1) AS bigint))')
