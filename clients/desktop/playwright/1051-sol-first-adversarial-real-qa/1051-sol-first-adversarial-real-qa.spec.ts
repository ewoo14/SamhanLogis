...
const SHOTS = path.resolve(dirname, '../../../../docs/qa/2026-08-11-1051-sol')
...
test('끊긴 재고 행은 수량·페이지·복사에 남고 UUID나 조작 표면은 생기지 않는다', async ({ page, context }) => {
  ...
  await expect(summary).toContainText('총 55건')
  ...
  expect(copied).toContain('HQ-001')
  ...
  await expect(page.getByTestId('inventory-balance-summary')).toContainText('총 55건')
