...
const r16RawDir = resolveQaShotsDir(path.join(root, 'docs/qa/2026-08-09-1154-r15/raw'))
...
function readR15Fixture(filename: string): Buffer {
  return fs.readFileSync(path.join(root, 'docs/qa/2026-08-09-1154-r15/raw', filename))
}
...
test('PR #1154 R16 실 HTTP와 관리자 화면에서 1000건 전량 확인', async ({ request, page, browserName }) => {
  ...
  const r16File = path.join(r16RawDir, '거래처-Excel다운로드_R16_BULK_1000.csv')
  fs.writeFileSync(r16File, Buffer.concat([readR15Fixture('거래처-Excel다운로드_R15_BULK_1000.csv'), Buffer.from('\n'.repeat(1 + (Date.now() % 997)), 'utf8')]))
  const r16BlankFile = path.join(r16RawDir, '거래처-Excel다운로드_R16_BLANK.csv')
  fs.writeFileSync(r16BlankFile, Buffer.concat([readR15Fixture('거래처-Excel다운로드_R15_BLANK.csv'), Buffer.from('\n'.repeat(1 + (Date.now() % 997)), 'utf8')]))
