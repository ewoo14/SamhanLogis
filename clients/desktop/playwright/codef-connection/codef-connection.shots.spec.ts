...
const QA_DIR = path.resolve(__dirname, '../../../../docs/qa/codef-task7')
 ...
 * data-testid='codef-connection-*' 구간에서 내부 식별자가 보이면 FAIL.
 */
...
for (const { label, width, height } of VIEWPORTS) {
  test.describe(`CodefConnectionPage ${label} 스크린샷`, () => {
    ...
    test(`[${label}] 01 초기 진입 — 빈 폼·빈 목록·결과 안내문구`, async ({ page }) => {
      ...
      // 기관 목록 테이블 빈 상태 메시지
      const institutionTable = page.getByTestId('codef-connection-institution-table')
    ...
    test(`[${label}] 02 폼 입력 — FormGrid 반응형·자격증명 입력`, async ({ page }) => {
      ...
      await expect(page.getByTestId('codef-connection-business-type')).toBeVisible()
      await expect(page.getByTestId('codef-connection-organization')).toBeVisible()
      await expect(page.getByTestId('codef-connection-login-type')).toBeVisible()
      await expect(page.getByTestId('codef-connection-credential-id')).toBeVisible()
      await expect(page.getByTestId('codef-connection-credential-password')).toBeVisible()

      // 기관코드 입력값 확인
      await expect(page.getByTestId('codef-connection-organization')).toHaveValue('0004')
