...
export function getMockResponse(config: AxiosRequestConfig): unknown | null {
  ...
  // GET /accounting/orders, /accounting/orders/{orderNo} — 회계 주문 조회 mock.
  const mockAccountingOrders = [
    {
      orderNo: '2026/05/04-1',
      partnerName: '삼한공조',
      managerName: '김영업',
      progressStatus: 'IN_PROGRESS',
      linkedSlipNo: '2026/05/04-7',
      validUntil: '2026-05-31',
      paymentTerms: '월말 마감 후 30일',
      reference: '견적서 Q-2026-0504-1',
      totalSupplyAmount: '3627273',
      totalVatAmount: '362727',
      totalAmount: '3990000',
      lines: [
        {
          lineNo: 1,
          itemName: '냉난방 실외기 (5HP)',
          quantity: '2',
          unitPrice: '1650000',
          supplyAmount: '3300000',
          vatAmount: '330000',
          lineTotal: '3630000',
          itemDueDate: '2026-05-20',
        },
        {
          lineNo: 2,
          itemName: '설치 부자재',
    ...
    {
      orderNo: '2026/05/31-3',
      partnerName: '한일빌딩',
      managerName: '박관리',
      progressStatus: 'COMPLETED',
      linkedSlipNo: '2026/05/31-4',
      validUntil: '2026-06-15',
      paymentTerms: '검수 완료 후 즉시',
      reference: '현장 추가 발주',
      totalSupplyAmount: '1090909',
      totalVatAmount: '109091',
      totalAmount: '1200000',
      lines: [
        {
          lineNo: 1,
          itemName: '환기유닛 ERV-200',
          quantity: '3',
          unitPrice: '363636',
          supplyAmount: '1090909',
          vatAmount: '109091',
          lineTotal: '1200000',
          itemDueDate: '2026-06-10',
        },
      ],
    },
  ]
  const accountingOrderDetailMatch = url.match(/\/accounting\/orders\/([^/?]+)$/)
  if (method === 'GET' && accountingOrderDetailMatch) {
    const orderNo = toSlashDocumentNo(decodeURIComponent(accountingOrderDetailMatch[1] ?? ''))
    const found = mockAccountingOrders.find((row) => row.orderNo === orderNo)
    return found ? envelope(found) : mockError(404, 'NOT_FOUND', '주문을 찾을 수 없습니다.')
  }
  if (method === 'GET' && /\/accounting\/orders(?:\?.*)?$/.test(url)) {
    const content = mockAccountingOrders.map(({ paymentTerms, reference, lines, ...summary }) => summary)
    const requestedSize = Number(config.params?.['size'] ?? content.length)
    const size = Number.isFinite(requestedSize) && requestedSize > 0 ? requestedSize : content.length
    return envelope({
      content,
      totalElements: content.length,
      totalPages: 1,
      number: Number(config.params?.['page'] ?? 0),
      size,
      first: true,
      last: true,
    })
