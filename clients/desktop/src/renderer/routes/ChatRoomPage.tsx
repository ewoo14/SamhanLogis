...
export function ChatRoomPage() {
  ...
  return <main data-testid="chat-room-page" style={{ display: 'grid', gap: 16, padding: 24 }}>
    ...
    {editingGroup ? <section aria-label="그룹방 편집" style={{ display: 'grid', gap: 8 }}><input aria-label="편집할 그룹방 이름" value={groupName} onChange={(event) => setGroupName(event.target.value)} /><input aria-label="편집 참여자 검색" value={editQuery} onChange={(event) => setEditQuery(event.target.value)} placeholder="이름 또는 담당자코드 검색" />{editRecipientsQuery.data?.map((recipient) => recipient.employeeCode ? <button type="button" key={recipient.userId} onClick={() => toggleEditParticipant(recipient.employeeCode!)}>{sanitizeInternalLabel(recipient.name)} · {recipient.department ?? ''} · {recipient.employeeCode}</button> : <span key={recipient.userId}>{sanitizeInternalLabel(recipient.name)} · {recipient.department ?? ''} · 담당자코드 미부여</span>)}<div aria-label="현재 참여자">{(groupRoom?.participants ?? []).filter((participant) => participant.employeeCode && groupCodes.includes(participant.employeeCode)).map((participant) => <button type="button" key={participant.employeeCode} onClick={() => toggleEditParticipant(participant.employeeCode!)} aria-label={`${sanitizeInternalLabel(participant.name)} 참여자 제거`}>{sanitizeInternalLabel(participant.name)} · {participant.employeeCode} ×</button>)}</div><div><Button type="button" onClick={() => editMutation.mutate()} disabled={editMutation.isPending || !groupName.trim() || groupCodes.length === 0}>저장</Button><Button type="button" onClick={() => setEditingGroup(false)}>취소</Button></div>{editError ? <p role="alert">담당자코드가 없는 직원은 참여자로 추가할 수 없습니다. 담당자코드 등록 후 다시 시도해 주세요.</p> : null}</section> : null}
