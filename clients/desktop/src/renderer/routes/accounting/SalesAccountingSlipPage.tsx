...
import {
  listSalesAccountingSlips,
  postSalesSlip,
  type SalesAccountingSlipResponse,
  type SalesAccountingSlipStatus,
...
export function SalesAccountingSlipPage() {
  ...
  const canPost = canAccess('accounting.sales-slip.accounting', 'update')
  const [from, setFrom] = useState(firstDayOfMonth())
  const [to, setTo] = useState(today())
  const [partnerCode, setPartnerCode] = useState('')
  const [status, setStatus] = useState<SalesAccountingSlipStatus | 'ALL'>('ALL')

  const query = useQuery({
    queryKey: ['sales-accounting-slips', from, to, partnerCode, status],
    queryFn: () =>
      listSalesAccountingSlips({
        from,
        to,
    ...
    <div data-testid="sales-accounting-slip-page">
      <Card style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
          ...
          />
          <select value={status} onChange={(e) => setStatus(e.target.value as SalesAccountingSlipStatus | 'ALL')} style={inputStyle}>
