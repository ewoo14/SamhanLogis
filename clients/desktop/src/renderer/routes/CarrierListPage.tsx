...
type Draft = { code: string; name: string; isArologis: boolean; partnerId: string }
const emptyDraft: Draft = { code: '', name: '', isArologis: false, partnerId: '' }
...
export function CarrierListPage() {
  ...
  const save = useMutation({
    mutationFn: () => editing
      ? carrierApi.update(editing.id ?? editing.code, { code: draft.code, name: draft.name, isArologis: draft.isArologis, partnerId: draft.partnerId || null })
      : carrierApi.create({ code: draft.code, name: draft.name, isArologis: draft.isArologis, partnerId: draft.partnerId || null }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-carriers'] }); setDraft(emptyDraft); setEditing(null) },
  })
  const toggle = useMutation({
    mutationFn: (carrier: Carrier) => carrierApi.update(carrier.id ?? carrier.code, { isActive: !carrier.isActive }),
  ...
  const columns: DataTableColumn<Carrier>[] = [
    ...
    { key: 'partner', header: '정산 거래처', render: (row) => <span>{row.partnerId ? '연결됨' : '미연결'}</span> },
    { key: 'active', header: '사용 여부', render: (row) => <Badge variant={row.isActive ? 'success' : 'neutral'}>{row.isActive ? '사용' : '중지'}</Badge> },
    { key: 'actions', header: '관리', render: (row) => <span style={{ display: 'flex', gap: 8 }}><Button size="sm" variant="secondary" onClick={() => { setEditing(row); setDraft({ code: row.code, name: row.name, isArologis: row.isArologis, partnerId: row.partnerId ?? '' }) }}>수정</Button><Button size="sm" variant="secondary" onClick={() => toggle.mutate(row)}>{row.isActive ? '사용 중지' : '사용 재개'}</Button></span> },
  ]
  return <main data-testid="carrier-list-page" style={{ padding: 24, display: 'grid', gap: 16 }}>
    ...
    <Card><form onSubmit={(event) => { event.preventDefault(); save.mutate() }} style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr) auto', gap: 12, alignItems: 'end' }}>
      ...
      <FormField label="정산 거래처 연결" render={() => <Input value={draft.partnerId} onChange={(e) => setDraft({ ...draft, partnerId: e.target.value })} placeholder="거래처 코드" />} />
