...
export function DispatchGroupPage() {
  ...
  const mutationError = [update, remove, assignCarrier, clearCarrier, addSlip].find((mutation) => mutation.isError)?.error
  ...
  const columns: DataTableColumn<DispatchGroup>[] = [
    ...
    { key: 'carrier', header: '운송사', render: (row) => <span>{row.carrierCode ? `${row.carrierCode} · ${row.carrierName ?? ''}` : '운송사 미지정'} {row.carrierCode && <Button type="button" onClick={() => clearCarrier.mutate(row.groupNo)}>지정 해제</Button>} {carriers.data?.filter((carrier) => carrier.isActive && carrier.code !== row.carrierCode).map((carrier) => <Button key={carrier.code} type="button" onClick={() => assignCarrier.mutate({ groupNo: row.groupNo, code: carrier.code })}>{carrier.code} 지정</Button>)}</span> },
    ...
    { key: 'actions', header: '조작', render: (row) => <span><Button type="button" onClick={() => update.mutate({ groupNo: row.groupNo, vehicleLabel: window.prompt('차량 표시명', row.vehicleLabel) ?? row.vehicleLabel })}>그룹 수정</Button> <Button type="button" onClick={() => remove.mutate(row.groupNo)}>그룹 삭제</Button></span> },
  ]
