...
export interface PricingStore {
  ...
  reset: () => void
  /**
   * 동일 카테고리 내 라인 순서 변경 — v2 정정 §정정 2 (품목 drag-and-drop).
   * activeId / overId 는 라인 UUID. 카테고리가 다른 라인은 무시.
   */
...
export const usePricingStore = create<PricingStore>((set, get) => ({
  ...
  reorderLines: (activeId, overId) => {
    ...
    set((state) => {
      const activeIdx = state.lines.findIndex((l) => l.id === activeId)
      const overIdx = state.lines.findIndex((l) => l.id === overId)
      if (activeIdx < 0 || overIdx < 0) return state
      // 카테고리 가드 — 다른 카테고리 사이로의 이동 무시 (UX 일관성).
      if (state.lines[activeIdx]!.category !== state.lines[overIdx]!.category) {
        return state
      }
