...
const OUTPUT_SLUG = process.env.QA_OUTPUT_SLUG || 'full-menu-real-qa-2026-06-03'
const REPORT_DATE = process.env.QA_REPORT_DATE || '2026-06-03'
const ROOT = path.resolve(__dirname, '../../..')
const QA_DIR = path.join(ROOT, 'docs', 'qa', OUTPUT_SLUG)
...
async function main() {
  ...
  async function fillOperationalSlipForm(kind) {
    ...
    async function pickAutocomplete(input, query, optionText, label) {
      await input.click()
      await input.fill(query)
      const option = page.locator('li[role=option]').filter({ hasText: optionText }).first()
      await option.waitFor({ state: 'visible', timeout: 12_000 })
      await option.click()
      await page.waitForTimeout(450)
      const value = await input.inputValue().catch(() => '')
      if (!value) throw new Error(`${kind}: ${label} autocomplete did not select a value`)
    }

    const warehouseInputs = page.locator('input[placeholder*="창고 코드"]')
    await pickAutocomplete(warehouseInputs.nth(0), 'HQ', 'HQ-001', isSales ? 'departure warehouse' : 'inbound warehouse')
    await pickAutocomplete(warehouseInputs.nth(1), isSales ? 'VH' : 'CS', isSales ? 'VH-001' : 'CS-001', isSales ? 'arrival warehouse' : 'source warehouse')

    await pickAutocomplete(
      page.locator('input[placeholder*="거래처명"]').first(),
      'P',
      'P-2026',
      'partner',
    )
    ...
    await pickAutocomplete(productInput, 'AR05', 'AR05', 'product')
  ...
  async function writeReport() {
    ...
    const report = `# 전체 메뉴 실사용자 QA 리포트 (${REPORT_DATE})

- 브랜치: \`${result.branch}\`
- 대상: \`${BASE_URL}\` + local Docker Desktop backend
- 산출 경로: \`docs/qa/${OUTPUT_SLUG}\`
...
- 상세 리포트: [docs/qa/${OUTPUT_SLUG}/REPORT.md](docs/qa/${OUTPUT_SLUG}/REPORT.md)

## 대표 스크린샷

![대시보드](docs/qa/${OUTPUT_SLUG}/screenshots/01-dashboard-after-login.png)
![매출 전표 생성](docs/qa/${OUTPUT_SLUG}/screenshots/flow-sales-02-created-list.png)
![매입 전표 삭제](docs/qa/${OUTPUT_SLUG}/screenshots/flow-purchases-07-after-delete.png)
![회계 전표 날짜 변경 실패](docs/qa/${OUTPUT_SLUG}/screenshots/flow-sales-accounting-02-date-changed.png)
