  ...
  React.useEffect(() => {
    ...
    async function runBootChecks() {
      async function observeOtaUpdate() {
        ...
        console.warn('[arologis-mobile-version] OTA 업데이트 확인 실패');
      ...
      try {
        ...
        console.warn('[arologis-mobile-version] 버전 정책 조회 실패');
        void observeOtaUpdate();
        if (mounted) {
      ...
      {otaSnapshot && (otaSnapshot.phase === 'downloading' || otaSnapshot.phase === 'available') ? (
        <View accessibilityRole="alert" style={styles.banner}>
          <View style={styles.bannerText}>
            <Text style={styles.bannerTitle}>{otaSnapshot.phase === 'downloading' ? '새 기사 앱 업데이트를 준비하는 중입니다' : '새 기사 앱 업데이트를 준비했습니다'}</Text>
            <Text style={styles.bannerBody}>{otaSnapshot.activity ? '현장 작업·서명·사진 처리를 보호하기 위해 작업이 끝난 뒤 자동으로 적용합니다.' : '잠시 후 앱을 안전하게 다시 시작해 적용합니다.'}</Text>
          </View>
        </View>
      ) : null}
      {failureMessage ? (
