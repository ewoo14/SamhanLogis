...
export function ProfileScreen(): JSX.Element {
  ...
  return (
    <ScreenContainer>
      ...
      {/* 정정 #12 — 거래처별 DC율 정보 표시 */}
      <RNCard>
        <Text style={styles.sectionTitle}>적용 DC율</Text>
        {dcConfig ? (
          <>
            <KV
              label="홈멀티 DC"
              value={dcConfig.homeMultiDc != null ? `${(dcConfig.homeMultiDc * 100).toFixed(0)}%` : '-'}
            />
            <KV
              label="상업멀티 DC"
              value={
                dcConfig.commercialMultiDc != null ? `${(dcConfig.commercialMultiDc * 100).toFixed(0)}%` : '-'
              }
            />
            <KV label="단가 처리" value={dcConfig.unitProcessing ? `${dcConfig.unitProcessing.toLocaleString('ko-KR')}원` : '-'} />
          </>
        ) : (
          <Text style={styles.dcEmpty}>DC 설정 정보가 없습니다 (정상가 적용).</Text>
        )}
      </RNCard>
...
const styles = StyleSheet.create({
  ...
  dcEmpty: { fontSize: fontSize.sm, color: colors.textSubtle, paddingVertical: spacing.sm },
});
