...
import { reformatLegacyOrderNumber } from '@/utils/formatSlipNumber';
      ...
      <RNCard>
        <View style={styles.headerRow}>
          <View style={styles.headerLeft}>
            ...
            <Text style={styles.orderNumber} testID="detail-order-number">
              {reformatLegacyOrderNumber(data.orderNumber || orderNumber)}
            </Text>
          ...
          {data.lines.map((line) => (
            <View key={line.id} style={styles.line}>
              <View style={styles.lineHead}>
                ...
                {/* 정정 #5 — '품명' → '품목명' (modelName 노출) */}
                <Text style={styles.modelCode}>{line.modelName}</Text>
              </View>
              {/* 정정 #4 — '모델 코드' → '모델명' (modelCode 보조 표시) */}
              <Text style={styles.modelName}>모델명: {line.modelCode}</Text>
