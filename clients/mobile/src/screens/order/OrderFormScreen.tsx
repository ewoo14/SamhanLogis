/**
 * OrderFormScreen v2 — 주문 작성 (legacy 모바일 분기 1:1).
 *
 ...
 *   - line 249~ : `.qty-input` 모바일 (height:32px; border-radius:4px)
 *   - line 260~ : `.est-table td.colD.mobile-only { text-align:left; padding-left:4px; font-weight:700 }`
 *
 * 본 v2 는 legacy 의 `<table class="est-table">` 모바일 분기 row 를 RN View/Text 로 1:1 변환.
 *
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {/* 배송 정보 카드 */}
        <View style={styles.card} testID="ship-card">
          <Text style={styles.cardTitle}>배송 정보</Text>
          <View style={styles.fieldGroup}>
            <Text style={styles.fieldLabel}>배송 주소</Text>
            <TextInput
          ...
          <View style={styles.fieldGroup}>
            <Text style={styles.fieldLabel}>인수자 연락처</Text>
            <TextInput
              style={styles.textInput}
        ...
        <Pressable
          style={({ pressed }) => [
            styles.submitBtn,
            (draft.lines.length === 0 || submit.isPending) && styles.submitBtnDisabled,
          ...
          testID="submit-order"
        >
          <Text style={styles.submitBtnText}>{submit.isPending ? '접수 중...' : '주문 접수'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
