/**
 * LegacyEstimateWebViewScreen v1 — react-native-webview 로 estimate-app v2 임베드 (영업직원용).
 *
 * 출처: Mobile v5 (`feature/migration-fe-mobile-v5-estimate` commit `6bf46bb`) 의
 *      LegacyEstimateWebViewScreen 패턴 1:1 — 인증 식별자만 partnerCode → employeeCode 분리.
 *
 * 사용자 명시 (PR #60 회고):
 *   - "견적서 → 삼한 사무실 영업직원용"
 *   - "추천대로 진행 (옵션 b 웹 모바일 뷰포트) / 단 앱 버전도 만들고 싶음"
 ...
 *   - shim 이 estimate-app v2 의 inline `google.script.run` shim 의 fetch 콜에 Authorization Bearer + X-Samhan-Staff 추가.
 *   - StaffLogin native 인증 token → `setEstimateAuthScript({...})` injectJavaScript 로 WebView 에 전달.
 ...
 *   - employeeCode (사번 e.g. "S001") 만 query parameter (`?email=S001@samhan-air.com`) 로 전달.
 */
...
/**
 * legacy estimate isMobileNow() trigger — userAgent 에 'samhan-staff' 마커 추가.
 * 결정적 분기는 `(max-width: 1280px)` matchMedia 이지만, UA 마커는 server-side 인지 보조용.
 */
const MOBILE_USER_AGENT_SUFFIX = ' SamhanStaffApp/0.1.0 (samhan-staff)';
...
export function LegacyEstimateWebViewScreen(): JSX.Element {
  ...
  // StaffLogin 의 employeeCode 를 estimate-app v2 의 ?email 자리로 전달 (estimate-app 는
  // userEmail 식별자로 권한/이력 조회). 운영 시 실 직원 이메일로 교체.
  const userEmailHint = employeeCode ? `${employeeCode}@samhan-air.com` : undefined;
  ...
  }, [shimReady, token, employeeCode, userEmailHint]);
  ...
  const handleMessage = useCallback((evt: WebViewMessageEvent) => {
    try {
      ...
      if (msg.type === 'shim-installed') {
        ...
        console.warn('[Legacy Estimate Staff] RPC error', msg.payload);
      } else if (msg.type === 'rpc-missing') {
        // eslint-disable-next-line no-console
        console.warn('[Legacy Estimate Staff] RPC missing — check ESTIMATE_RPC_INVENTORY', msg.payload);
      ...
      // eslint-disable-next-line no-console
      console.warn('[Legacy Estimate Staff] message parse failed', e);
    }
  }, [nav]);
  ...
  const handleHelp = useCallback(() => {
    Alert.alert(
      ...
      [
        ...
        `URL: ${initialUri}`,
        employeeName ? `영업직원: ${employeeName} (${employeeCode ?? '-'})` : '',
        mobileModeActive === null ? '' : `mobile-mode: ${mobileModeActive ? '활성' : '비활성'}`,
      ]
        .filter(Boolean)
        .join('\n'),
    );
  }, [initialUri, employeeName, employeeCode, mobileModeActive]);
