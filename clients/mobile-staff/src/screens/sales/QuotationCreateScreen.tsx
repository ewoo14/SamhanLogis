/**
 * QuotationCreateScreen — P1-4 영업 native 앱 신규 견적 작성.
 *
 * 단계:
 *   1. 거래처 선택 (CustomerSearchScreen 임베드 / 선택 후 표시)
 *   2. 라인 추가 (품목 코드 + 품목명 + 수량 + 단가)
 *   3. 합계 확인 후 제출
 *
 * BE: quotation-service POST /api/v1/quotations/mobile
 *     (@PreAuthorize SALES/MANAGER/MASTER)
 *
 * UUID 비공개:
 *   - 화면에 표시: partnerCode + partnerName + productCode + quotationNo.
 *   - 내부 전송: partnerId (UUID) → API body, 응답 quotationId 는 저장하지 않음.
 */
...
import { createMobileQuotation, type CustomerSummary, type QuotationLineRequest, type QuotationResponse } from '../../api/sales';
...
interface LineForm {
  id: string;          // 로컬 key (UUID 아님 — Math.random)
  productCode: string;
  productName: string;
  quantity: string;    // TextInput = 문자열; 제출 시 숫자 변환
  unitPrice: string;
}
...
export default function QuotationCreateScreen({ token, onBack }: Props): JSX.Element {
  ...
  const removeLine = useCallback((id: string) => {
    setLines(prev => {
      if (prev.length <= 1) return prev; // 최소 1라인 유지
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      const linePayload: QuotationLineRequest[] = lines.map(l => ({
      ...
      const res = await createMobileQuotation(
        {
      ...
      const msg = e instanceof Error ? e.message : '견적 생성 중 오류가 발생했습니다.';
      Alert.alert('오류', msg);
    } finally {
      setSubmitting(false);
    }
  }, [selectedCustomer, lines, validDays, memo, token]);

  // ---- 렌더 ----
  if (step === 'customer') {
    return (
      <View style={styles.container}>
        <ScreenHeader title="신규 견적 — 거래처 선택" onBack={onBack} />
  ...
  if (step === 'done' && result) {
    return (
      <View style={styles.container}>
        <ScreenHeader title="견적 작성 완료" onBack={onBack} />
        <View style={styles.doneBox}>
          <Text style={styles.doneTitle}>견적이 생성되었습니다.</Text>
          {/* UUID 비공개 — quotationId 노출 금지, quotationNo 만 표시 */}
          <InfoRow label="견적 번호" value={result.quotationNo} />
          <InfoRow label="거래처" value={result.partnerName} />
          <InfoRow label="합계" value={`${result.totalAmount.toLocaleString('ko-KR')}원`} />
          <InfoRow label="상태" value={statusLabel(result.status)} />
          <TouchableOpacity
  ...
  return (
    ...
    >
      <ScreenHeader title="신규 견적 — 라인 입력" onBack={() => setStep('customer')} />
      ...
      >
        ...
        {/* 라인 목록 */}
        <Text style={styles.sectionTitle}>견적 라인</Text>
        <FlatList
          data={lines}
          keyExtractor={l => l.id}
          scrollEnabled={false}
        ...
        <View style={styles.totalRow}>
          <Text style={styles.totalLabel}>합계 (부가세 별도)</Text>
          <Text style={styles.totalValue}>{total.toLocaleString('ko-KR')}원</Text>
        </View>

        {/* 메모 */}
        <FormGroup label="특이사항 (선택)">
          <TextInput
        ...
        >
          {submitting ? (
            ...
            <Text style={styles.submitBtnLabel}>견적 생성</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
...
function LineFormRow({ item, index, onUpdate, onRemove, canRemove }: LineFormRowProps): JSX.Element {
  return (
...
function statusLabel(status: QuotationResponse['status']): string {
  const MAP: Record<QuotationResponse['status'], string> = {
    DRAFT: '초안',
    SENT: '발송됨',
    CONFIRMED: '확정',
    CANCELLED: '취소',
