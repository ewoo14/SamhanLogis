/**
 * Playwright capture script — mobile-staff v1 QA 캡처 5장.
 *
 * v1 = react-native-webview 로 estimate-app v2 (Node + Express + EJS) 임베드 + 영업직원 인증.
 * playwright 로 expo web export (dist) 를 띄워 StaffLogin native + 3-tab BottomTab + 견적 WebView
 * placeholder 진입 흐름 + 프로필 / 로그아웃을 캡처.
 *
 * 산출물 (390x844, iPhone 14 Pro):
 *   docs/qa/migration-fe-mobile-staff-v1/01-staff-login.png
 *   docs/qa/migration-fe-mobile-staff-v1/02-staff-home.png
 *   docs/qa/migration-fe-mobile-staff-v1/03-staff-estimate-webview.png
 *   docs/qa/migration-fe-mobile-staff-v1/04-staff-estimate-webview-after-add.png
 *   docs/qa/migration-fe-mobile-staff-v1/05-staff-profile.png
 *
 * 실행:
 *   1) npx expo export --platform web   (dist 생성)
 *   2) node scripts/capture-v1.cjs
 *
...
const OUT_DIR = path.resolve(__dirname, '../../../docs/qa/migration-fe-mobile-staff-v1');
...
/**
 * 정적 server (axios import 회피 — Node http 만).
 *
 * dist 디렉토리 안 파일을 path 매핑 — 미존재 시 index.html (SPA fallback).
 */
...
async function mockApi(page) {
  // mobile-staff staff-login mock — S001/1234 통과.
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .staff-tag{position:fixed;top:6px;left:8px;background:#1e40af;color:#fff;font-size:10px;padding:2px 8px;border-radius:9999px;font-weight:700;z-index:10}
</style></head>
<body class="mobile-mode">
<div class="staff-tag">영업 S001</div>
...
async function withPage(browser, fn) {
  const ctx = await browser.newContext({
    ...
    deviceScaleFactor: 1,
    userAgent:
      'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1 SamhanStaffApp/0.1.0 (samhan-staff)',
  });
...
async function performStaffLogin(page) {
  const codeInput = page.locator('[data-testid="staff-employee-code-input"]');
  if ((await codeInput.count()) > 0) {
    await codeInput.first().fill('S001');
    await page.waitForTimeout(150);
    const pwInput = page.locator('[data-testid="staff-password-input"]');
    if ((await pwInput.count()) > 0) {
      await pwInput.first().fill('1234');
      await page.waitForTimeout(150);
    }
    const submit = page.locator('[data-testid="staff-submit"]');
...
(async () => {
  ...
  try {
    // 01 — StaffLogin (사번 + 비밀번호 native)
    await withPage(browser, async (page) => {
      await gotoApp(page);
      // 사번 + 비밀번호 입력 (제출 전 캡처 — 입력 상태 시각화)
      const codeInput = page.locator('[data-testid="staff-employee-code-input"]');
      if ((await codeInput.count()) > 0) {
        await codeInput.first().fill('S001');
        await page.waitForTimeout(150);
        const pwInput = page.locator('[data-testid="staff-password-input"]');
        if ((await pwInput.count()) > 0) await pwInput.first().fill('1234');
        await page.waitForTimeout(300);
      }
      await snapshot(page, '01-staff-login.png');
    });

    // 02 — 영업직원 환영 + 견적 진입 button (3 tabs BottomTab)
    await withPage(browser, async (page) => {
      await gotoApp(page);
      await performStaffLogin(page);
      await snapshot(page, '02-staff-home.png');
    });

    // 03 — 견적 tab → estimate-app v2 WebView (mobile-mode 활성, 4 카드 stack)
    await withPage(browser, async (page) => {
      await gotoApp(page);
      await performStaffLogin(page);
      await clickMenuTab(page, '견적');
      await injectEstimateOverlay(page, /* showLines */ false, /* afterAdd */ false);
      await snapshot(page, '03-staff-estimate-webview.png');
    });
    ...
    await withPage(browser, async (page) => {
      ...
      await snapshot(page, '04-staff-estimate-webview-after-add.png');
    });

    // 05 — 영업직원 프로필 + 로그아웃 button
    await withPage(browser, async (page) => {
      await gotoApp(page);
      await performStaffLogin(page);
      await clickMenuTab(page, '프로필');
      await snapshot(page, '05-staff-profile.png');
    });

    console.log('\nmobile-staff v1 QA capture 5장 완료 →', OUT_DIR);
  } finally {
    await browser.close();
    server.close();
  }
