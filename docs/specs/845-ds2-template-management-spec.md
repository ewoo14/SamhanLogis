...
| D-DS2-02 | **버전 = status 라이프사이클 `DRAFT`/`ACTIVE`**(에픽 명문·번호 revision 상태머신 아님). **낙관락 ≠ revision 분리**(R2 신규): `lock_version BIGINT @Version`(JPA 자동·모든 update·409 원천) **별도 컬럼·응답 미노출**(R3: DS-2는 내부 tx 충돌 검출만·If-Match/ETag 없음·DS-3 편집기 stale-save 방지는 read-only version/ETag 후속) + `revision INT`(정보성·`updateDocument()`=DRAFT document 편집서만 수동 +1). rename/activate는 revision 불변. **과거 문서 pin=DS-3 무결성 결정**(§7·DS-2는 커스텀 저작 불가로 무의미) | B5·R2-B1 |
