# #845 DS-2 — 문서 양식 템플릿 영속·활성 렌더 (기획 spec v3 · OPUS 4.8)

- 에픽: #845 문서 양식 디자이너 · 파일럿=결재 문서 · 브랜치 `feat/845-ds2-template-management` · PR #847
- 기준일: 2026-07-18 · **v2 = SOL R1 NO-GO(B5·H8·M3·L1) 반영** · **v3 = SOL R2 NO-GO(신규 B2·H2·M3) 반영**(revision/@Version 분리·schema_version 권위·activate 대상제외·docType backfill·라운드트립 BE E2E·API JSON표·상한 수치)
...
| D-DS2-02 | **버전 = status 라이프사이클 `DRAFT`/`ACTIVE`**(에픽 명문·번호 revision 상태머신 아님). **낙관락 ≠ revision 분리**(R2 신규): `lock_version BIGINT @Version`(JPA 자동·모든 update·409 원천) **별도 컬럼** + `revision INT`(정보성·`updateDocument()`=DRAFT document 편집서만 수동 +1). rename/activate는 revision 불변. **과거 문서 pin=DS-3 무결성 결정**(§7·DS-2는 커스텀 저작 불가로 무의미) | B5·R2-B1 |
...
| D-DS2-04 | **활성 불변식 = docType당 ACTIVE ≤1**(0=정상·DEFAULT fallback). partial unique `(doc_type) WHERE status='ACTIVE' AND is_deleted=false`. `activate(id)`=**bulk 강등 `WHERE doc_type=:dt AND status='ACTIVE' AND id<>:targetId`**(대상 제외·R2 교정: 대상 미제외 시 in-memory ACTIVE dirty 미발생→활성 0 구멍)+flush 후 승격·**이미 ACTIVE면 idempotent 성공**·경합/unique→**409**. IT 3종: activate(현 ACTIVE)·동일 ID 동시·상이 ID 동시(실 Postgres) | H1·R2-H3 |
| D-DS2-05 | **docType 권위 = `ApprovalLine.documentType`**(영속·포맷 `GROUPWARE_${code}`·`documentTypeFor` L158). `ApprovalLineAdminResponse`에 `documentType` **노출 추가**→렌더러 조회 키. **V10 backfill**(R2 신규): `UPDATE approval_lines SET document_type='GROUPWARE_'||t.code FROM approval_templates t WHERE template_id=t.id AND document_type IS NULL`(비활성/soft-del 템플릿도 raw SQL이 봄·FE active-list를 레거시 권위로 쓰지 않음). null→DEFAULT. **`DocumentTemplate.doc_type VARCHAR(40)`=source(`ApprovalLineBase.document_type` 40) 정합** | B2·R2-H1 |
...
- **라운드트립 게이트(2단계·R2 교정: FE mock만은 실 Postgres JSONB 왕복 미증명=false-green)**: **(1단계·BE 소유)** Testcontainers HTTP E2E — `GROUPWARE_DEFAULT` payload POST→activate→active GET, 반환 DocumentPayload가 입력과 바이트동일 assert + **canonical active-response artifact 커밋**. **(2단계·FE 소유)** frozen-golden 테스트가 그 artifact를 parse→렌더 === frozen golden 바이트동일. 실행: BE `gradlew :groupware-service:test`·FE `npm run test`.
