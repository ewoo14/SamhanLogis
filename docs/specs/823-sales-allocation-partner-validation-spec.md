# #823 매출·매입전표 배분 원천 거래처 검증 (기획 spec v3 · OPUS 4.8)

- 이슈: #823 · 브랜치 `feat/823-sales-allocation-partner-validation` · PR #849 · 결정=**불일치 차단 reject**([[project_pending_decisions_2026_07_19]])
- 기준일: 2026-07-19 · **v2 = SOL 기획검수 NO-GO(B1·H5·M5·L1) 반영 · v3 = SOL 재검수 GO(LOW 4 folded: readiness 계약·snapshot 직접저장·헤더검증 INVALID_INPUT400·preflight is_deleted+#850)**
...
| D-823-02 | **배포 순서 계약 = producer(slip) → consumer(accounting)**(SOL-B). consumer-first면 구 producer 응답 partnerId 부재→null→**전 배분 reject**(D-823-04와 상호작용)이라 위험. accounting record `@JsonIgnoreProperties(ignoreUnknown=true)`는 양방향 안전 아님·순서 계약 필수. **readiness = contract readiness**(단순 health 아님·실 `getSlipLine/getSlipLines` 응답에 non-null partnerId 존재 + 구 producer 인스턴스 부재 확인) deploy runbook 명시 | SOL-BLOCKING·R2-LOW |
...
| D-823-05 | **원천 identity 권위 = 스냅샷 직접 저장**(SOL-H·R2 권장): client `sourceSlipId/sourceSlipNo` 신뢰 시 line은 A전표 소속인데 payload가 B전표 지칭하는 분열 가능. **`getSlipLine(sourceLineId)` 스냅샷의 `slipId/slipNo`를 저장 권위로 채택**(strict 일치 거부안은 fixture drift 유발 → snapshot 직접 저장이 기존 DB/응답 계약과 무충돌). 거래처 검증도 스냅샷 기준 | SOL-HIGH·R2 |
| D-823-06 | **대상 헤더 거래처 필수 검증 선행**(SOL-M): `CreateSalesAccountingSlipRequest.partnerId` `@NotNull`·`@Valid` 부재. **`createDraftAttempt` 진입 직후·number 생성/원천 조회 전에 헤더 partner 필수 검증 = `INVALID_INPUT(400)` 단일 코드**(누락→"원천 불일치" 오진 방지). 매출·매입 동일 | SOL-MED·R2 |
...
- **착수 preflight**(SOL-M): 배포 대상 환경 `SELECT count(*) FROM slips WHERE status='CONFIRMED' AND partner_id IS NULL AND is_deleted=false` = 0 확인(활성행 한정·삭제 legacy 무시) + 보정 runbook. (실측 로컬: 활성 null 1940[DRAFT 1926·SENT 13·REJECTED 1]·**CONFIRMED null 0**·배분가능 OUTBOUND 4/INBOUND 1 → 현 과차단 0. SENT 13은 partner 없이 CONFIRMED 전이 가능 주의.)
...
- **[별건·over-allocation 계열·등록 완료 #850]** 동일 요청 내 같은 원천 라인 중복 배분 시 요청 내 누적 미반영(원천100에 60+60 통과) — #823(거래처) 범위 밖·별개 결함 → **이슈 #850 등록됨**([[feedback_fix_in_current_pr_no_split]]).
