...
logging-service 선택 기동 (기본 런처의 profile에는 포함되지 않음):
...
logging-service만 올린다. `scripts/launch-local-stack.ps1/.sh`의 기본 `base + local-all`
합성 기동은 logging profile을 활성화하지 않으므로 logging-service를 생성하지 않는다.
이 PC에서는 influxd가 host 8086을
