...
단, 개발책임자 지적에 따라 최초 QA 산출물의 범위 표현을 보정합니다. 이번 산출물은 Samhan Public Desktop 전체 메뉴 실 Docker QA이며, 전체 프로젝트 QA라고 부르려면 Samhan Public 14 service, 5 client, 아로로지스 독립 운영 단위, role matrix, service-per-DB 정합성까지 선행 기준선으로 문서화하고 추가 검증해야 합니다. 보정 기준은 `PROJECT-QA-BASELINE.md`에 정리했습니다.
