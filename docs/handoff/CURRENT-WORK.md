...
| PR | 내용 | CI | 남은 것 |
|---|---|---|---|
| #1223 | 전환 흐름 · 사이드바 중복 정리 | 41/41 | 재수렴 검증 중 |
| #1229 | 주문서웹 창고 = 품목분류 | 42/42 | 🚩 개발책임자 확인 3건 ↓ |
| #1230 | 일마감 다중선택 + 정렬/필터 | 40/41 | 적대검증 중 |
| #1231 | 잘못된 식별자 500 → 400/405 | 37/40 | 적대검증 중 |
| #1162 | 자격 로테이션 | GitGuardian 1 | 🚩 **머지 보류** ↓ |
| #1188 | 바로빌·알리고 | — | ⏸️ 자격 확보 전 |

### 🚩 #1229 개발책임자 확인 3건

```text
① 미분류 품목을 어느 창고로 — 지금은 "걸리지 않은 것" 으로 보고 초월 00003
   활성 상품 3084 중 catM 없음 2194(71%) 이라 크게 작용한다
② 레거시와 갈리는 상품 32개 (상일→초월 27 · 초월→상일 5) — 목록은 보고서에
③ 상품 마스터에 없는 모델이 섞이면 확정이 실패한다
   가격 계산 방어선(6a219fa8a7)이 의도한 동작이다. 여는 것이 맞는가?
   ⟹ 아니면 그 모델 2개(AR05TXEAAWKNEU-11 · AR15TXEAAWKNEU-07)를 마스터에 넣는 것이 답
```
