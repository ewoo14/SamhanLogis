...
### 실행 순서 (PM 자율)
1. 회계체인: **#823 ✅ 머지 완료**(main `3fee4b699`·PR #849)→**전표 거래처 필수화(전이 가드·결정완료)**→**#825 슬5**  2. #825 슬6(쪽지칩)·슬7(주문병합)  3. **#845 DS-3**→DS-4  4. #824(품목 VAT)·**#848 ◀ 현재 진행 중**  5. chore 배치(#831·#832·#838·#839·#828)  6. AC 후속(#834·836·837·840·842·843)  7. 🔴 **Google 자격 후** #827·#773  8. Phase11 #830 · cutover-defer #826

> **🔵 #848 documentType 3-저장소 확장 — 현재 진행 슬라이스**(PR #852·브랜치 feat/848-documenttype-column-widen): **기획 spec v4 GO**(SOL 기획검수 v1→v4 수렴) → **CODEX LUNA 구현**(groupware approval_lines V11·document_templates.doc_type V11·auth approval_line_config V89·shared ApprovalLineBase·FE templateSchema 모두 40→70) → **OPUS R1 5-agent**(HIGH: DocumentTemplateCreate/UpdateRequest `@Size(40)` 미확장→실 HTTP 400·라이브 확증→OPUS fix DTO 70+anti-false-green MockMvc IT) → **CODEX SOL R2 5-agent**(테스트 정확값 단언·mock 70·CI skipped=0 게이트·docs sync·DTO 한글메시지 등→LUNA fix). **라이브 QA 완결**: 3컬럼 실 write 경로 41-70 저장 실증(①발의→approval_lines 65자 ②문서양식 POST/activate/active-GET 65자 ③auth addStep 65자). **v1(단일컬럼) NO-GO 교훈**: auth 도 GROUPWARE_${code} 실경로(라이브 DB·grep false-negative)·ddl-validate 는 length 미검사(→information_schema 단언)·**64 NULL=template_id-less(backfill 0행)**. R2 fix 후 재수렴→PM종합→CI green→머지 예정.
>
