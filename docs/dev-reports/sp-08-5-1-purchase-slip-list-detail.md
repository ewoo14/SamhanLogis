...
| 검증 | 명령 | 결과 |
|---|---|---|
| RED | `.\gradlew.bat :services:slip-service:test --tests "*SlipQueryPurchase*" --no-daemon --rerun-tasks` | FAIL 3건 (`type` alias, INVENTORY 403, `inspectionStatus`) |
| GREEN | `.\gradlew.bat :services:slip-service:test --tests "*PurchaseSlip*" --tests "*SlipQueryPurchase*" --no-daemon --rerun-tasks` | PASS, 5 tests |
