# PR #1145 R11 — projection을 V99 반영 상태로 정합

## 판정

V99을 변경하지 않고, 체크인 projection을 저장소의 전체 Flyway migration 적용 결과로
...
`samhan-postgres/auth_db`에 `docker exec` SELECT를 실행했다. 공유 DB에는 V99이 아직
...
`flyway/flyway:10.10.0`으로 새 DB에 적용한다. 실행 결과는 99 migrations, v99이다.
...
줄어든 5셀은 이전 projection에서 `0000000`이었고 mock은 V99 효과를 이미 갖고 있었다.
전체 5셀을 projection에 반영했기 때문에 이 5개가 divergence에서 제거됐다. 나머지
349셀은 mock을 수정하지 않고 그대로 동결했다. 최종 실측은 `DIVERGENCE_COUNT=349`다.

## ③ 변경된 5셀 전수 및 V99 대조

| 역할 | page code | 이전 비트 | 이후 비트 | V99 근거 |
|---|---|---:|---:|---|
| MANAGER | `accounting.tax-invoice.inbound.manage` | `0000000` | `1111000` | V99 §5, 139행 주석 및 template INSERT 159행. V14 MANAGER `accounting.tax-invoice.inbound`를 `.inbound.manage`로 계승 |
| MANAGER | `accounting.sales-slip.accounting` | `0000000` | `1111000` | V99 §2, 39행 주석, CASE 46행·source 54행. MANAGER `.list`를 `.accounting`으로 계승 |
| MANAGER | `accounting.purchase-slip.accounting` | `0000000` | `1111000` | V99 §2, 39행 주석, CASE 47행·source 54행. MANAGER `.list`를 `.accounting`으로 계승 |
| SALES | `accounting.sales-slip.accounting` | `0000000` | `1000000` | V99 §2, 39행 주석, CASE 46행·source 54행. SALES `.list`를 `.accounting`으로 계승 |
| SALES | `accounting.purchase-slip.accounting` | `0000000` | `1000000` | V99 §2, 39행 주석, CASE 47행·source 54행. SALES `.list`를 `.accounting`으로 계승 |

다섯 셀 모두 V99의 명시된 role/template 계승 또는 inbound.manage 이관과 일치한다.
V99과 무관한 셀은 없으므로 중단 조건은 발생하지 않았다.
...
해당 테스트의 Flyway 로그는 `Successfully applied 99 migrations ... now at version v99`를
...
두 mutation 모두 정합 이후에도 감시 가드가 RED가 됨을 확인했다. mock bit와 V99은
변경하지 않았다.
...
- `clients/desktop/src/renderer/test-utils/permission-mock-divergences.ts`
  — 354셀에서 V99과 정합된 5셀 제거, 349셀 exact 동결
