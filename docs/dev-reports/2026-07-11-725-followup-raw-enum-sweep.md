...
## 후속 분리(별도 이슈)
1. 타 서비스 raw enum(inventory `StockTransfer/StockInstance/InventoryAudit/InboundInspection`·partner `PartnerCreditService`·partner-auth·notification) — 신규 SSOT 4개 동반.
2. Tier2 non-enum 500마스킹(inventory `InventoryAuditService`·product `ProductCatalogController`·auth `ApprovalLineConfig`) + ApiResponse 래퍼.
3. PartnerOrderPrintService 인쇄라벨 SSOT 통합(design intent 확인) · UUID interpolation sweep · docs/qa 증적 재생성.
