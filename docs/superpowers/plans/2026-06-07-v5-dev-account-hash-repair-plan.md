...
## 1. 배경
V5 seed 9계정의 공통 해시 `$2a$12$6cxHjNrguvlnEE...` 는 문서된 평문 "dev_p05_pass!" 와 **불일치** (#411 QA `bcrypt.checkpw=False` 실증, `docs/qa/permission-groups-phase-c-fullstack/real-qa-evidence.md:38`). 이후 실QA 마다 psql 수동 UPDATE 로 우회 — 재기동/재시드 시 재발. #421(V48)은 검증 해시 `$2b$12$g9/AnrEr4...` 로 비전파 완료, **V5 기존 계정은 잔존**.
...
3. QA Docker: dev_sales/dev_master "dev_p05_pass!" 실 로그인 200 (psql 우회 없이) 실측.
