...
| 드리프트 8건 목록 | survey §5 표 + s1 dev-report §13.4 | 8건 중 **2건이 해소되어 6건이 된다** | 문서 갱신 범위 = §7 |

### 2.1 드리프트 8건 → 6건 (해소되는 2건)
...
⚠️ 이 조합은 예외가 아니라 **가장 흔한 경로**다 — `show_i_hose=true` 인 30개 거래처에서 담당자가 화면칩을 따로 켜지 않으면 전부 여기에 해당한다.
...
| 드리프트 6건 | `H-01`·`H-07`·`H-01-I-DOM-ONLY`·`C-01-AIR-PANEL`·`S-01-CATEGORY-DRIFT`·`C-02-REMAINDER-DRIFT`·`C-09-2812` | 범위 밖 |
...
4. **본다** — `유연호스 I형 1WAY` 행 수량이 **2**, `유연호스 I형 4WAY` 행 수량이 **1**. 상업멀티 합계 금액에 그 2개가 반영돼 있다. → 스크린샷
