...
1. **서로 다른 두 mutation 이 같은 entity 에서 같은 revision_no 를 갖지 않는다** — 채번을 per-entity DB **advisory lock** 으로 직렬화(다중 인스턴스 `MAX+1` 원자성). ~~안전망 = `(entity_id, revision_no, field_name)` partial UNIQUE.~~ ⚠️ **정정**(구현 중 발견): `(entity_id, revision_no)` 단독 UNIQUE 는 아래 #2(다중 필드 batch)와 모순이라 폐기 — 실측 14 batch group. 🚨 **추가 정정**(개발책임자 결정, 2026-07-27, PR #947 1차 적대검증): field-level `(entity_id, revision_no, field_name)` UNIQUE(V67) 도 기존 `DepositMatchAuditRecorder`(actorId 를 entity_id 로 재사용 + revision 하드코딩 1/2 직접 save)와 충돌해 **폐기** — **advisory lock 단독**으로 유일성을 보장한다(DB UNIQUE 안전망 없음).
...
DB 권위 채번 중 택1: ① per-entity advisory lock + DB max+1 ② DB sequence ③ `(entity_id, revision_no)` UNIQUE 제약(Flyway V, partial `WHERE is_deleted=false`) + INSERT 충돌 시 재조회·재시도. ~~**③의 UNIQUE 제약은 어느 수단을 쓰든 안전망으로 반드시 추가**(중복이 조용히 저장되지 않게).~~ 🚨 **정정**(개발책임자 결정, 2026-07-27): UNIQUE 안전망(V67)은 `DepositMatchAuditRecorder` 채번 방식과 충돌해 폐기 — **① advisory lock 단독** 채택, DB UNIQUE 안전망 없이 유일성을 보장한다.

## 수용 기준

- ~~`(entity_id, revision_no)` 활성 UNIQUE 제약(Flyway 신규 V) 존재.~~ 🚨 **폐기**(개발책임자 결정, 2026-07-27) — advisory lock 단독으로 유일성 보장, DB UNIQUE 안전망 없음.
