# 주문→출고전표 전환 시 재고 예약 정합 — 설계 (Phase 2.6c)

> grounding 완료(2026-05-30, backend-engineer agent). 전환 경로 재고 미연동 확인 → 예약(reserve) + 사전차단 구축.
> ⚠️ Codex 토큰 소진 → 2026-06-01(월) 12:00 복구 전까지 구현+dual리뷰 Claude 에이전트 전면 대체.

## 0. ⚠️ 모델 정정 (2026-05-31, 개발책임자 확정 — 본 정정이 이하 본문보다 우선)
- **주문서 = 재고 무영향.** 거래처가 보낸 주문이 우리 주문 리스트에 있어도, 견적→주문(DRAFT)이어도, 그 자체로는 재고에 손도 대지 않는다.
- **우리가 "출고전표로 전환(convert)" 해야 = 재고 예약(reserve) 발생.** 실재고 차감(deduct)이 아니라 **예약**이다. 예약은 가용재고를 묶고 실재고는 유지한다.
- **실재고 차감(deduct) = 실제 출고확정 단계 = 본 슬라이스 범위 제외(후속).**
- **재고 조회 = 가용재고 / 실재고 / 예약재고 구분 표시.** (가용 = 실재고 − 예약)
- **예약 가능 수량 부족 시 전환 409 사전차단.**
- **confirm(거래처 포털 주문확정 → 자동 slip 발행)의 "주문확정 시 reserve" 제거** — 주문 무영향 원칙. confirm 자동발행 자체 폐지는 2.6b. 본 슬라이스는 reserve 제거만(과도기 dev-report 명시).
- ⇒ 이하 본문에서 "deduct/차감"으로 적힌 부분은 모두 **"reserve/예약"** 으로 대체하여 읽을 것. inventory-service 에 이미 reserve/release 도메인 존재 → deduct 신규 불필요, reserve 에 추적/멱등 보강.
