...
SELECT (:partner_total::bigint = 1000 AND :partner_deleted::bigint = 1000
       ...
       (:slip_slips_deleted::bigint = 295
        AND :slip_lines_deleted::bigint = 636) AS slip_deleted_state,
       (:slip_slips_deleted::bigint = 0 AND :slip_lines_deleted::bigint = 0) AS slip_restored_state
\gset state_
...
\echo 'slip_db: slips deleted-marker=' :slip_slips_deleted ' (0이면 복구 상태 후보)'
\echo 'slip_db: lines deleted-marker=' :slip_lines_deleted ' (0이면 복구 상태 후보)'
  ...
  \if :repair_requested
    \if :repair_confirmed
      \if :repair_restore_partner
        ...
        SELECT COUNT(*) = 1000 AS repaired FROM partners
        WHERE partner_code ~ '^SOL1154R20-BULK-[0-9]+$'
          AND biz_no = partner_code AND NOT is_deleted AND deleted_by IS NULL \gset
        \if :repaired
          COMMIT;
      ...
      \elif :repair_restore_slip
        ...
        BEGIN;
        SELECT pg_advisory_xact_lock(hashtext('qa-residue-soft-delete-2026-08-12'));
        UPDATE slip_lines SET is_deleted = FALSE, deleted_at = NULL, deleted_by = NULL
        WHERE is_deleted AND deleted_by = 'qa-residue-softdelete-2026-08-12';
        UPDATE slips SET is_deleted = FALSE, deleted_at = NULL, deleted_by = NULL, deleted_by_name = NULL
        WHERE is_deleted AND deleted_by = 'qa-residue-softdelete-2026-08-12';
