...
- 🚨🚨 **2026-08-12 개발책임자 정정 — 적대검증도 codex 토큰으로** ([feedback_pm_delegate_to_codex_conserve_tokens.md](.claude/memory/feedback_pm_delegate_to_codex_conserve_tokens.md)) — *"OPUS에 왜 적대검증을??" · "코덱스 토큰만 사용해"*. 위 캐논의 **"1차 적대검증 = OPUS 4.8 서브에이전트" 는 폐지**. **Agent 서브에이전트는 PM 세션과 같은 토큰 풀**이라 절약이 0 이다. 정찰·1차/2차/재수렴 적대검증·대조 = **CODEX SOL**(`gpt-5.6-sol`), 구현·라운드 fix = **CODEX LUNA**(`gpt-5.6-luna`). 🚫 적대검증·정찰 목적의 **Agent 툴 발주 금지**. 리뷰어/구현자 분리는 모델이 아니라 **스테이지**로 유지(SOL 검증 → LUNA fix · SOL 을 구현 폴백으로 쓰지 않음).
